...
eastl::function<void( int width, int height )>GUI_onWindowSizeChange = nullptr;

static eastl::function<bool(SDL_Event* ev)> user_handle_events = nullptr;
...
//20191618
static eastl::vector<GUI_UserMessage> user_message_queue;
...
int GUI_Init( const char* title, int expectedWidth, int expectedHeight ) {
	...
	int style = SDL_WINDOW_RESIZABLE;
...
void GUI_Run(eastl::function<bool(SDL_Event* ev)> user_handle_ev) {
...
static void GUI_Loop() {
    if( GUI_View::closeQueue.size() ) {
        for (eastl::vector<GUI_View *>::iterator it = GUI_View::closeQueue.end()-1 ; it >= GUI_View::closeQueue.begin(); --it) {
            GUI_View *child = *it;
...
GUI_View *GUI_createTopView(const char* t, int x, int y, int w, int h,
	eastl::function<bool(SDL_Event* ev)>userEventHandler) {
    if (GUI_topView) {
