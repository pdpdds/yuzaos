...
#include <stat_def.h>
static int W_FileLengthByFileName(char* filename)
{
   struct stat fileinfo;
   if(fstat(filename,& fileinfo) != 0)
  ...
  if (strlen(filename)<=4 || strcasecmp(filename+strlen(filename)-4, ".wad" ))
    {
      ...
      singleinfo.size = LONG_(W_FileLengthByFileName(filename));
  ...
  else
    {
      ...
      header.numlumps = LONG_(header.numlumps);
      header.infotableofs = LONG_(header.infotableofs);
      length = header.numlumps*sizeof(filelump_t);
      fileinfo2free = fileinfo = malloc(length);    // killough
      fseek(fp, header.infotableofs, SEEK_SET);
      fread(fileinfo, 1, length, fp);
    ...
    for (i=startlump ; i<numlumps ; i++,lump_p++, fileinfo++)
      {
        lump_p->handle = (int)fp;                    //  killough 4/25/98
        lump_p->position = LONG_(fileinfo->filepos);
        lump_p->size = LONG_(fileinfo->size);
...
void W_ReadLump(int lump, void *dest)
{
  ...
  if (l->data)     // killough 1/31/98: predefined lump data
  ...
  else
    {
      ...
      I_BeginRead();
      fseek((FILE*)l->handle, l->position, SEEK_SET);
      c = fread(dest, 1, l->size, (FILE*)l->handle);
      if (c < l->size)
...
void WritePredefinedLumpWad(const char *filename)
{
   ...
   if((file = fopen(fn, "wb")))
   {
      ...
      header.numlumps     = LONG_(num_predefined_lumps);
      header.infotableofs = LONG_(sizeof(header));
      ...
      for(i = 0; i < num_predefined_lumps; i++)
      {
         ...
         fileinfo.filepos = LONG_(filepos);
         fileinfo.size    = LONG_(predefined_lumps[i].size);         
