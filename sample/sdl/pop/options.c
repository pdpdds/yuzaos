...
#ifndef _WIN32
char g_strExternlPath[256] = "./prince/";
#endif // !_WIN32
...
void load_global_options() {
    ...
    //snprintf(custom_path_buffer, max_len, "%s/popforandroid/mods/%s/%s", g_strExternlPath, levelset_name, quick_file /*QUICKSAVE.SAV*/ );
    snprintf(custom_quick_path, POP_MAX_PATH, "%s/popforandroid/%s", g_strExternlPath, "SDLPoP.ini" );

    ini_load(custom_quick_path, global_ini_callback); // global configuration
    load_dos_exe_modifications("."); // read PRINCE.EXE in the current working directory
#endif
}
...
//	struct stat info;
	...
	if (fp != NULL && SDL_RWsize(fp) > 0) {
		dos_version = identify_dos_exe_version(SDL_RWsize(fp));
	} else {
	...
	if (dos_version >= 0) {
		...
		byte* exe_memory = malloc((size_t)SDL_RWsize(fp));
		SDL_RWread(fp, exe_memory, (size_t)SDL_RWsize(fp), 1);
...
#define process(x, nbytes, ...) \
		do { \
			...
			read_ok = read_exe_bytes(x, nbytes, exe_memory, offset, SDL_RWsize(fp)); \
		} while(0)
...
void load_mod_options() {
	...
	if (use_custom_levelset) {
		...
		bool ok = false;
//		struct stat info;
		//if (stat(folder_name, &info) == 0) {
			//if (S_ISDIR(info.st_mode)) {
				// It's a directory
				ok = true;
				strncpy(mod_data_path, folder_name, sizeof(mod_data_path));
				// Try to load PRINCE.EXE (DOS)
				load_dos_exe_modifications(folder_name);
				// Try to load mod.ini
				char mod_ini_filename[POP_MAX_PATH];
				snprintf(mod_ini_filename, sizeof(mod_ini_filename), "%s/%s", folder_name, "mod.ini");
