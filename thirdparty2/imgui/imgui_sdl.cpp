...
namespace
{
	...
	namespace TupleHash
	{
		...
		template <typename T> void CombineHash(size_t& seed, const T& value)
		{
			seed ^= TupleHash::Hash<T>()(value) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
		}

		template <typename Tuple, size_t Index = eastl::tuple_size<Tuple>::value - 1> struct Hasher
		{
			static void Hash(size_t& seed, const Tuple& tuple)
			{
				Hasher<Tuple, Index - 1>::Hash(seed, tuple);
				CombineHash(seed, eastl::get<Index>(tuple));
			}
		};
		...
		template <typename Tuple> struct Hasher<Tuple, 0>
		{
			static void Hash(size_t& seed, const Tuple& tuple)
			{
				CombineHash(seed, eastl::get<0>(tuple));
			}
		};

		template <typename... T> struct Hash<eastl::tuple<T...>>
		{
			size_t operator()(const eastl::tuple<T...>& value) const
			{
				size_t seed = 0;
				Hasher<eastl::tuple<T...>>::Hash(seed, value);
				return seed;
			}
		};
	}

	template <typename Key, typename Value, size_t Size> class LRUCache
	{
	public:
		...
		void Insert(const Key& key, Value value)
		{
			...
			Container.insert(eastl::make_pair(key, Order.begin()));
	...
	private:
		...
		eastl::hash_map<Key, decltype(Order.begin()), TupleHash::Hash<Key>> Container;
	};
	...
	struct Device
	{
		...
		using UniformColorTriangleKey = eastl::tuple<uint32_t, int, int, int, int, int, int>;
		...
		using GenericTriangleVertexKey = eastl::tuple<int, int, double, double, uint32_t>;
		using GenericTriangleKey = eastl::tuple<GenericTriangleVertexKey, GenericTriangleVertexKey, GenericTriangleVertexKey>;

		LRUCache<UniformColorTriangleKey, TriangleCacheItem*, UniformColorTriangleCacheSize> UniformColorTriangleCache;
		LRUCache<GenericTriangleKey, TriangleCacheItem*, GenericTriangleCacheSize> GenericTriangleCache;
	...
	struct Rect
	{
		...
		{
			float minX = smallest(v0.pos.x, v1.pos.x, v2.pos.x);
			float minY = smallest(v0.pos.y, v1.pos.y, v2.pos.y);
			float maxX = biggest(v0.pos.x, v1.pos.x, v2.pos.x);
			float maxY = biggest(v0.pos.y, v1.pos.y, v2.pos.y);
			
			float minUVX = smallest(v0.uv.x, v1.uv.x, v2.uv.x);
			float minUVY = smallest(v0.uv.y, v1.uv.y, v2.uv.y);
			float maxUVX = biggest(v0.uv.x, v1.uv.x, v2.uv.x);
			float maxUVY = biggest(v0.uv.y, v1.uv.y, v2.uv.y);
			return Rect{ minX, minY, maxX, maxY, minUVX, minUVY, maxUVX, maxUVY };
			
			/*return Rect{
				eastl::min<float>({ v0.pos.x, v1.pos.x, v2.pos.x }),
				eastl::min({ v0.pos.y, v1.pos.y, v2.pos.y }),
				eastl::max({ v0.pos.x, v1.pos.x, v2.pos.x }),
				eastl::max({ v0.pos.y, v1.pos.y, v2.pos.y }),
				eastl::min({ v0.uv.x, v1.uv.x, v2.uv.x }),
				eastl::min({ v0.uv.y, v1.uv.y, v2.uv.y }),
				eastl::max({ v0.uv.x, v1.uv.x, v2.uv.x }),
				eastl::max({ v0.uv.y, v1.uv.y, v2.uv.y })
			};*/
		}
	};
	...
	struct FixedPointTriangleRenderInfo
	{
		...
		static FixedPointTriangleRenderInfo CalculateFixedPointTriangleInfo(const ImVec2& v1, const ImVec2& v2, const ImVec2& v3)
		{
			...
			const int x1 = static_cast<int>(roundf(v1.x * scale));
			const int x2 = static_cast<int>(roundf(v2.x * scale));
			const int x3 = static_cast<int>(roundf(v3.x * scale));

			const int y1 = static_cast<int>(roundf(v1.y * scale));
			const int y2 = static_cast<int>(roundf(v2.y * scale));
			const int y3 = static_cast<int>(roundf(v3.y * scale));

			int minX = smallest(x1, x2, x3);
			minX = (minX + 0xF) >> 4;
			int maxX = biggest(x1, x2, x3);
			maxX = (maxX + 0xF) >> 4;
			int minY = smallest(y1, y2, y3);
			minY = (minY + 0xF) >> 4;
			int maxY = biggest(y1, y2, y3);
			maxY = (maxY + 0xF) >> 4;
	...
	void DrawTriangleWithColorFunction(const FixedPointTriangleRenderInfo& renderInfo, const eastl::function<Color(float x, float y)>& colorFunction, Device::TriangleCacheItem* cacheItem)
	{
	...
	void DrawTriangle(const ImDrawVert& v1, const ImDrawVert& v2, const ImDrawVert& v3, const Texture* texture)
	{
		...
		const auto key = eastl::make_tuple(
			eastl::make_tuple(static_cast<int>(roundf(v1.pos.x)) - renderInfo.MinX, static_cast<int>(roundf(v1.pos.y)) - renderInfo.MinY, v1.uv.x, v1.uv.y, v1.col),
			eastl::make_tuple(static_cast<int>(roundf(v2.pos.x)) - renderInfo.MinX, static_cast<int>(roundf(v2.pos.y)) - renderInfo.MinY, v2.uv.x, v2.uv.y, v2.col),
			eastl::make_tuple(static_cast<int>(roundf(v3.pos.x)) - renderInfo.MinX, static_cast<int>(roundf(v3.pos.y)) - renderInfo.MinY, v3.uv.x, v3.uv.y, v3.col));
		...
		auto cached = new Device::TriangleCacheItem();
	...
	void DrawUniformColorTriangle(const ImDrawVert& v1, const ImDrawVert& v2, const ImDrawVert& v3)
	{
		...
		const auto key = eastl::make_tuple(v1.col,
			static_cast<int>(roundf(v1.pos.x)) - renderInfo.MinX, static_cast<int>(roundf(v1.pos.y)) - renderInfo.MinY,
			static_cast<int>(roundf(v2.pos.x)) - renderInfo.MinX, static_cast<int>(roundf(v2.pos.y)) - renderInfo.MinY,
			static_cast<int>(roundf(v3.pos.x)) - renderInfo.MinX, static_cast<int>(roundf(v3.pos.y)) - renderInfo.MinY);
