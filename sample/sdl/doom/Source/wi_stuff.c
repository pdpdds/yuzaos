...
// NET GAME STUFF
#define NG_STATSY     50
#define NG_STATSX     (32 + SHORT_(star->width)/2 + 32*!dofrags)
...
static void WI_drawLF(void)
{
  ...
  // draw <LevelName> 
  V_DrawPatch((SCREENWIDTH - SHORT_(lnames[wbs->last]->width))/2,
              y, FB, lnames[wbs->last]);

  // draw "Finished!"
  y += (5*SHORT_(lnames[wbs->last]->height))/4;
  
  V_DrawPatch((SCREENWIDTH - SHORT_(finished->width))/2,
              y, FB, finished);
}
...
static void WI_drawEL(void)
{
  ...
  // draw "Entering"
  V_DrawPatch((SCREENWIDTH - SHORT_(entering->width))/2,
              y, FB, entering);

  // draw level
  y += (5*SHORT_(lnames[wbs->next]->height))/4;

  V_DrawPatch((SCREENWIDTH - SHORT_(lnames[wbs->next]->width))/2,
  ...
  patch_t*  c[] )
{
  ...
  do
    {
      left = lnodes[wbs->epsd][n].x - SHORT_(c[i]->leftoffset);
      top = lnodes[wbs->epsd][n].y - SHORT_(c[i]->topoffset);
      right = left + SHORT_(c[i]->width);
      bottom = top + SHORT_(c[i]->height);
...
( int   x,
  ...
  int   digits )
{
  int   fontwidth = SHORT_(num[0]->width);
  int   neg;
  int   temp;
  ...
  if (t <= 61*59)  // otherwise known as 60*60 -1 == 3599
    {
      ...
      do
        {
          n = (t / div) % 60;
          x = WI_drawNum(x, y, n, 2) - SHORT_(colon->width);
          div *= 60;
    ...
    {
      // "sucks"
      V_DrawPatch(x - SHORT_(sucks->width), y, FB, sucks); 
    }
}
...
static void WI_drawDeathmatchStats(void)
{
  ...
  V_DrawPatch(DM_TOTALSX-SHORT_(total->width)/2,
  ...
  for (i=0 ; i<MAXPLAYERS ; i++)
    {
      if (playeringame[i])
        {
          V_DrawPatch(x-SHORT_(p[i]->width)/2,
                      DM_MATRIXY - WI_SPACINGY,
                      FB,
                      p[i]);
      
          V_DrawPatch(DM_MATRIXX-SHORT_(p[i]->width)/2,
                      y,
                      FB,
                      p[i]);
...
static void WI_drawNetgameStats(void)
{
  ...
  int   x;
  int   y;
  int   pwidth = SHORT_(percent->width);
  ...
  // draw stat titles (top line)
  V_DrawPatch(NG_STATSX+NG_SPACINGX-SHORT_(kills->width),
              NG_STATSY, FB, kills);

  V_DrawPatch(NG_STATSX+2*NG_SPACINGX-SHORT_(items->width),
              NG_STATSY, FB, items);

  V_DrawPatch(NG_STATSX+3*NG_SPACINGX-SHORT_(secret->width),
              NG_STATSY, FB, secret);
  
  if (dofrags)
    V_DrawPatch(NG_STATSX+4*NG_SPACINGX-SHORT_(frags->width),
                NG_STATSY, FB, frags);

  // draw stats
  y = NG_STATSY + SHORT_(kills->height);
  ...
  for (i=0 ; i<MAXPLAYERS ; i++)
    {
      ...
      x = NG_STATSX;
      V_DrawPatch(x-SHORT_(p[i]->width), y, FB, p[i]);

      if (i == me)
        V_DrawPatch(x-SHORT_(p[i]->width), y, FB, star);
