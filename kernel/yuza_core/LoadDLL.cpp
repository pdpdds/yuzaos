...
static ELoadDLLResult LoadDLL_SetSectionMemoryProtection(LOAD_DLL_CONTEXT* ctx)
{
	...
	*/
	...
	for (i=0,s=ctx->sect; i<ctx->hdr.FileHeader.NumberOfSections; ++i,++s)
	{
		...
		if (!kVirtualProtect((LPVOID)(ctx->image_base + s->VirtualAddress), s->Misc.VirtualSize, protection, &oldProtection))
		...
		try
		{
			...
			finally
			{
				if (!finished_successfully)
					kVirtualFree(ctx.image, 0, MEM_RELEASE);
				...
				if (!finished_successfully || !info)
				{
					...
					{						
						for (int i = 0; i < (int)ctx.num_import_modules; ++i)
							UnloadDLL((MODULE_HANDLE)(ctx.loaded_import_modules_array[i].image_base));
...
{
	if (ModuleManager::GetInstance()->IsSystemPE(info->moduleName))
		return true;
	...
	if (info->loaded_import_modules_array)
	{
		...
		{
			UnloadDLL((MODULE_HANDLE)(info->loaded_import_modules_array[i].image_base));
		}
	...
	kVirtualFree(info->mem_block, 0, MEM_RELEASE);
	return res;
}
...
static bool LoadDLLFromFileCallback(void* buff, size_t position, size_t size, LOAD_DLL_FROM_FILE_STRUCT* param)
{
	...
	int readResult = fread(buff, 1, size, param->f);
