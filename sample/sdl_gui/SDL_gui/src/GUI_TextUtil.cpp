...
int GUI_GetPreviousUTF8Index(eastl::string str, int i ) {
    if( i > 0 && i <= str.length() ) {
        while( i > 0 ) {
            i--;
            int c = (str.at(i));
...
int GUI_GetPreviousMainUTF8Index(eastl::string str, int i ) {
    if( i > 0 && i <= str.length() ) {
        int byte_count = 0;
        int byte = 0;
        while( i > 0 ) {
...
int GUI_GetUnicodeAtIndex(eastl::string str, int i, int *nextIndex ) {
    if( i >= 0 && i < str.length() ) {
        int byte_count = 0;
        int byte = 0;
        while( i < str.length() ) {
