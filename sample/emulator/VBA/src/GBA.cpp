...
int CPULoadRom(const char* szFile)
{
	...
	pReserve = (u8*)Syscall_VirtualAlloc(NULL, 0x2000000, (0x00002000), 0x04);
	rom = (u8*)Syscall_VirtualAlloc(pReserve, 0x2000000, (0x00001000), 0x04);

	if (rom == NULL)
	{
		systemMessage(MSG_OUT_OF_MEMORY, N_("Failed to allocate memory for %s"), "ROM");
		return 0;
	}
