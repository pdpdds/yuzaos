...
_tiffSeekProc(thandle_t fd, uint64 off, int whence)
{
	...
	offli.u.LowPart=SetFilePointer(fd, offli.u.LowPart,&offli.u.HighPart,dwMoveMethod);
	if ((offli.u.LowPart ==INVALID_SET_FILE_POINTER)&&(Syscall_GetLastError()!=0))
		offli.QuadPart=0;
...
{
	ULARGE_INTEGER m;
	m.u.LowPart=GetFileSize(fd,&m.u.HighPart);
	return(m.QuadPart);
}
