            ...
            if ((copy_mode != CONCAT) && (source->flags & CI_BINARY)) {
                ...
                if (!DestIsDevice) {
                    ...
                    if (VerifyCurrent) {
                       ...
                       SetFilePointer( srcptr,  0, NULL, FILE_BEGIN);
                       SetFilePointer( destptr, 0, NULL, FILE_BEGIN);

                       dwSrcFileSize  = GetFileSize( srcptr,  &dwSrcFileSizeHigh);
                       dwDestFileSize = GetFileSize( destptr, &dwDestFileSizeHigh);
                       ...
                       while (!fEof) {
                           ...
                           if (!ReadFile(srcptr, buf_seg, buf_len, &bytes_read,NULL ) ) {
                               Cclose(destptr);
                               destptr=BADHANDLE;
                               goto l_out;
                           }
    ...
    )
{
    ...
    if (fOutputUnicode)
#endif // UNICODE
        return WriteFile(fh, rgb, cb, lpcb, NULL);
#ifdef UNICODE
    else {
    ...
    )
{
    ...
    *pdestptr = Copen_Copy2(curr_dest, (ULONG)O_RDONLY);
    ...
    SetFilePointer( *pdestptr, - (LONG) bytes_read, NULL, FILE_END);

    if (!ReadFile(*pdestptr, buf_seg_dest, bytes_read, &bytes_read_dest, NULL) ) {
        Cclose(*pdestptr);
        *pdestptr=BADHANDLE;
        PutStdErr(MSG_VERIFY_FAIL, ONEARG, curr_dest);
        return (FAILURE);
    ...
    *pdestptr = Copen2((TCHAR *)curr_dest,
    ...
    SetFilePointer( *pdestptr, 0, NULL, FILE_END);
