      ...
      if ((*port == '\0' || *endp != '\0') || port_num < 0 || port_num >= 65536)
        {
          g_set_error (error,
                       G_IO_ERROR,
                       G_IO_ERROR_INVALID_ARGUMENT,
                       _("Error in address ¡°%s¡± ¡ª the port attribute is malformed"),
                       address_entry);
          goto out;
        }
    }
  ...
  if (family != NULL && !(g_strcmp0 (family, "ipv4") == 0 || g_strcmp0 (family, "ipv6") == 0))
    {
      g_set_error (error,
                   G_IO_ERROR,
                   G_IO_ERROR_INVALID_ARGUMENT,
                   _("Error in address ¡°%s¡± ¡ª the family attribute is malformed"),
                   address_entry);
      goto out;
    }
      ...
      if ((path == NULL && abstract == NULL) || (path != NULL && abstract != NULL))
        {
          g_set_error (error,
                       G_IO_ERROR,
                       G_IO_ERROR_INVALID_ARGUMENT,
                       _("Error in address ¡°%s¡± ¡ª the unix transport requires exactly one of the "
      ...
      if (host == NULL)
        {
          g_set_error (error,
                       G_IO_ERROR,
                       G_IO_ERROR_INVALID_ARGUMENT,
                       _("Error in address ¡°%s¡± ¡ª the host attribute is missing or malformed"),
                       address_entry);
          goto out;
        }
      ...
      if ((*s == '\0' || *endp != '\0') || port < 0 || port >= 65536)
        {
          g_set_error (error,
                       G_IO_ERROR,
                       G_IO_ERROR_INVALID_ARGUMENT,
                       _("Error in address ¡°%s¡± ¡ª the port attribute is missing or malformed"),
                       address_entry);
          goto out;
        }
          ...
          if (nonce_file == NULL)
            {
              g_set_error (error,
                           G_IO_ERROR,
                           G_IO_ERROR_INVALID_ARGUMENT,
                           _("Error in address ¡°%s¡± ¡ª the noncefile attribute is missing or malformed"),
                           address_entry);
              goto out;
            }
        }
