...
*/
    ...
    return strcmp(fh->_name, "STDIN") == 0 || strcmp(fh->_name, "STDOUT") == 0;
    ...
    )
{
    ...
    va_start(arg_ptr,fmt);              /* setup arg_ptr for decoding  */
     ...
     _vsntprintf(MsgBuf,MAXCBMSGBUFFER,fmt,arg_ptr);
    ...
    if (bytesread > 0)
    {
        ...
        if (FileIsConsole(STDOUT)) {
            if (!(flag=WriteConsole(STDOUT, MsgBuf, bytesread, &byteswrit, NULL)))
                rc = GetLastError();
        }
        else {
            bytesread *= sizeof(TCHAR);
            flag = MyWriteFile(         /* write file data         */
                (CRTHANDLE)STDOUT,                 /* device handle  */
                (CHAR *)MsgBuf,         /* buffer                  */
                ...
                byteswrit != bytesread)   /* not write all the data then   */
        {
            ...
            if (FileIsDevice((CRTHANDLE)STDOUT))     /* if file is device     */
            {                             /*                               */
