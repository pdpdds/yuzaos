                     ...
                     int height, byte *palette)
{
  ...
  pcx->xmin = 0;
  pcx->ymin = 0;
  pcx->xmax = SHORT_((short)(width-1));
  pcx->ymax = SHORT_((short)(height-1));
  pcx->hres = SHORT_((short)width);
  pcx->vres = SHORT_((short)height);
  memset (pcx->palette,0,sizeof(pcx->palette));
  pcx->color_planes = 1;        // chunky image
  pcx->bytes_per_line = SHORT_((short)width);
  pcx->palette_type = SHORT_(2); // not a grey scale
                     ...
                     int height, byte *palette)
{
  ...
  bmfh.bfType = SHORT_(19778);
  bmfh.bfSize = LONG_(fhsiz+ihsiz+256L*4+width*height);
  bmfh.bfReserved1 = SHORT_(0);
  bmfh.bfReserved2 = SHORT_(0);
  bmfh.bfOffBits = LONG_(fhsiz+ihsiz+256L*4);

  bmih.biSize = LONG_(ihsiz);
  bmih.biWidth = LONG_(width);
  bmih.biHeight = LONG_(height);
  bmih.biPlanes = SHORT_(1);
  bmih.biBitCount = SHORT_(8);
  bmih.biCompression = LONG_(BI_RGB);
  bmih.biSizeImage = LONG_(wid*height);
  bmih.biXPelsPerMeter = LONG_(0);
  bmih.biYPelsPerMeter = LONG_(0);
  bmih.biClrUsed = LONG_(256);
  bmih.biClrImportant = LONG_(256);
