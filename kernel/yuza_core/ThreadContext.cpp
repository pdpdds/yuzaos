...
void ThreadContext::SetupThread(THREAD_START_ENTRY startAddress, void *param, unsigned int userStack, unsigned int kernelStack)
{
	...
	if (fKernelThread) 
	{
		...
		PUSH(fStackPointer, param); // param
		PUSH(fStackPointer, kTerminateThread); // return address
...
void ThreadContext::UserThreadStart(unsigned int startAddress, unsigned int userStack, unsigned int param)
{
	//ThreadParam* Param = (ThreadParam*)param;
	//kprintf("ThreadContext::userthreadStart %s %x %x\n", (char*)Param->param, Param->entryPoint, userStack);
	/*char* name = (char*)param;
	userStack = (userStack - strlen(name) - 1);
	memcpy(reinterpret_cast<void*>(userStack), name, strlen(name) + 1);*/

	userStack = (userStack - sizeof(ThreadParam));
	memcpy((ThreadParam*)(userStack), (ThreadParam*)param, sizeof(ThreadParam));
	//memcpy((void*)userStack, Param, sizeof(ThreadParam));
	//CopyUser((void*)userStack, (void*)param, sizeof(ThreadParam));

	//Param = (ThreadParam*)userStack;
	//kprintf("ThreadContext::userthreadStart %s %x %x\n", (char*)Param->param, Param->entryPoint, userStack);
	...
	// cleanly.
	// movl $3, %eax; int $50
	/*const unsigned char kExitStub[] = { 0xb8, 19, 0, 0, 0, 0xcd, 0x32 };
	userStack = (userStack - sizeof(kExitStub));
	...
	*/
	...
	//PUSH(userStack, argAddress);				// Param for user start func
	//PUSH(userStack, userStack + 8);		// Return address (the exit stub)
