...
void HUlib_drawTextLine(hu_textline_t *l, boolean drawcursor)
{
  ...
  // killough 1/18/98 -- support multiple lines
  if (drawcursor && x + SHORT_(l->f['_' - l->sc]->width) <= SCREENWIDTH)
...
void HUlib_eraseTextLine(hu_textline_t* l)
{
  ...
  if (!automapactive && viewwindowx && l->needsupdate)
    {
      int yoffset, lh = SHORT_(l->f[0]->height) + 1;
                     ...
                     int startchar, char *cr, boolean *on)
{
  ...
  s->cl = 0;
  for (i=0;i<h;i++)
    HUlib_initTextLine(s->l+i, x, y - i*(SHORT_(font[0]->height)+1),
                       font, startchar, cr);
}
...
static void HUlib_eraseMBg(hu_mtext_t *m)
{
  ...
  if (!automapactive && viewwindowx)
    {
      int yoffset, lh = SHORT_(m->l[0].f[0]->height) + 1;
