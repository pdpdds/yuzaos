...
// We use the C++11 Standard Library mutex as-is.
	...
	// static_assert(EA_ALIGN_OF(mMutexBuffer) >= EA_ALIGN_OF(CRITICAL_SECTION), "mMutexBuffer alignment failure"); //
	// Enabling this causes the VS2012 compiler to crash.

#if !defined(_WIN32_WINNT) || (_WIN32_WINNT < 0x0403)
		//InitializeCriticalSection((CRITICAL_SECTION*)mMutexBuffer);
		Syscall_CreateMutex(mMutex);
	...
	{
#if defined(EA_PLATFORM_MICROSOFT)
		Syscall_LockMutex(mMutex);
#elif defined(EA_PLATFORM_POSIX)
		pthread_mutex_lock(&mMutex);
#else
		EASTL_FAIL_MSG(
		    "EASTL thread safety is not implemented yet. See EAThread for how to do this for the given platform.");
#endif
	}
	...
	shared_ptr_auto_mutex::shared_ptr_auto_mutex(const void* /*pSharedPtr*/) : auto_mutex(*gSharedPtrMutex.get()) {}
