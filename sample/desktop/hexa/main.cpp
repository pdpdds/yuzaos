...
int main(int argc, char** argv)
{
	...
	Syscall_CreateWindow(&rect, "Hexa", WINDOW_FLAGS_DEFAULT, &qwWindowID);
	...
	Syscall_DrawText(&qwWindowID , &point, &textColor,
	...
	while (1)
	{
		...
		if ((g_stGameInfo.bGameStart == TRUE) &&
			((Syscall_GetTickCount() - qwLastTickCount) >
...
void DrawInformation(QWORD qwWindowID)
{
	...
	// 임시 버퍼에 출력할 정보를 저장
	sprintf(vcBuffer, "Level: %ld, Score: %ld\n", g_stGameInfo.qwLevel,
		g_stGameInfo.qwScore);
...
void DrawGameArea(QWORD qwWindowID)
{
	...
	rect.left = 0;
	rect.top = iY;
	rect.right = BLOCKSIZE * BOARDWIDTH;
	rect.bottom = iY + BLOCKSIZE * BOARDHEIGHT;
	...
	for (j = 0; j < BOARDHEIGHT; j++)
	{
		for (i = 0; i < BOARDWIDTH; i++)
		{
			...
			if (g_stGameInfo.vvbBoard[j][i] != EMPTYBLOCK)
			{
				...
				rect.top = iY + (j * BLOCKSIZE);
				rect.right = (i + 1) * BLOCKSIZE;
				rect.bottom = iY + ((j + 1) * BLOCKSIZE);
				...
				rect.right = (i + 1) * BLOCKSIZE;
				rect.bottom = iY + ((j + 1) * BLOCKSIZE);
				Syscall_DrawRect(&qwWindowID, &rect, stColor, FALSE);

				stColor = g_stGameInfo.vstEdgeColor[g_stGameInfo.vvbBoard[j][i]];

				rect.left = i * BLOCKSIZE + 1;
				rect.top = iY + (j * BLOCKSIZE) + 1;
				rect.right = (i + 1) * BLOCKSIZE - 1;
				rect.bottom = iY + ((j + 1) * BLOCKSIZE) - 1;
				Syscall_DrawRect(&qwWindowID, &rect, stColor, FALSE);
			}
		}
	}
	...
	if (g_stGameInfo.iBlockX != -1)
	{
		for (i = 0; i < BLOCKCOUNT; i++)
		{
				...
				(iY + ((g_stGameInfo.iBlockY + i) * BLOCKSIZE)))
			{
				...
				rect.left = g_stGameInfo.iBlockX * BLOCKSIZE;
				rect.top = iY + ((g_stGameInfo.iBlockY + i) * BLOCKSIZE);
				rect.right = (g_stGameInfo.iBlockX + 1) * BLOCKSIZE;
				rect.bottom = iY + ((g_stGameInfo.iBlockY + i + 1) * BLOCKSIZE);
				Syscall_DrawRect(&qwWindowID, &rect, stColor, TRUE);
				...
				rect.left = g_stGameInfo.iBlockX * BLOCKSIZE + 1;
				rect.top = iY + ((g_stGameInfo.iBlockY + i) * BLOCKSIZE) + 1;
				rect.right = (g_stGameInfo.iBlockX + 1) * BLOCKSIZE - 1;
				rect.bottom = iY + ((g_stGameInfo.iBlockY + i + 1) * BLOCKSIZE) - 1;
				Syscall_DrawRect(&qwWindowID, &rect, stColor, FALSE);
			}
		}
	}
	...
	rect.left = 0;
	rect.top = iY;
	rect.right = BLOCKSIZE * BOARDWIDTH - 1;
	rect.bottom = iY + BLOCKSIZE * BOARDHEIGHT - 1;
