...
namespace Gwk
{
    ...
    struct Texture
    {
        typedef eastl::list<Texture*> List;
...
//namespace std
//{
    template<> struct eastl::hash<Gwk::Font>
    {
        ...
        {
            result_type res = eastl::hash<Gwk::String>{}(f.facename);
    ...
    template<> struct eastl::hash<Gwk::Texture>
    {
        ...
        result_type operator()(argument_type const& f) const noexcept
        {
            return eastl::hash<decltype(f.name)>{}(f.name);
        }
    };
//} // namespace std
