...
/*void my_mouse_init()
{
	...
	{
		int svgalib_type = vga_getmousetype();

		has_mouse = 1;
		if (svgalib_type == MOUSE_NONE)
		{
			has_mouse = 0;
			fprintf(stderr, "zgv: svgalib reports no mouse configured!\n"
				"     (possibly an error in the config file?)\n"
				"     carrying on anyway...\n");
			Syscall_Sleep(3);		// bit annoying but it's an important problem!
		}
		else
		{
			mouse_fd = mouse_init_return_fd("/dev/mouse",
				svgalib_type, MOUSE_DEFAULTSAMPLERATE);
			if (mouse_fd == -1)
				has_mouse = 0;
		}
	}
	...
	if (has_mouse)
	{
		...
		// ditch any byte which appears in next 1/20th-sec.
		 // this is to workaround a problem with my `mouse' (a logitech trackball)
		 // which sends a bogus `N' byte on initialisation for no obvious reason.
		 // (This screws everything up because input is *meant* to be in packets
		 //of 3 bytes!)
		 //
		 // 1/20-th sec is roughly five times as long as it should take,
		 // so this should work unless the machine is impossibly overloaded.
		...
		// set scaling of mouse x/y. Default is 16, which gives a fast-ish
		 // mouse on my trackball, and a slower but acceptable result with
		 // another mouse I tried.
		 //
		mouse_setscale(cfg.mousescale);

		// set mouse pos to centre of screen - we only do this once
		found = 0;
		for (md_ptr = modedesc; md_ptr->mode; md_ptr++)
			if (fs_vgamode == md_ptr->mode)
			{
				mouse_setposition(md_ptr->width / 2, md_ptr->height / 2);
				found = 1;
				break;
			}
		...
		{
			fprintf(stderr, "zgv: "
				"unknown file-sel video mode, can't correctly init mouse position\n");
			mouse_setposition(320, 240);		// should at least be onscreen :-)
			/* not desperately nasty though, so carry on.
		}

		// not too nice not having scrollbar if using mouse, so force it on.
		cfg.scrollbar = 1;
	}
}*/
	...
	for (y = 0; y < height; y++)
		for (x = 0; x < width; x++)
			if (pixelsize == 1)
			{
				...
				printf("%c", image_palette[0 + c]);
				printf("%c", image_palette[256 + c]);
				printf("%c", image_palette[512 + c]);
			}
			else
			{
				/* just copy */
				printf("%c", theimage[(y * width + x) * 3 + 2]);
				printf("%c", theimage[(y * width + x) * 3 + 1]);
				printf("%c", theimage[(y * width + x) * 3]);
			}
}
...
void load_one_file(char* filename)
{
	...
	{
		hf = NULL;
		if (!cfg.writefile) printf( "Loading...");
	}
	...
	if (readpicture(filename, hf, !cfg.writefile, 0, NULL, NULL) != _PIC_OK)
	{
		...
		printf("\rzgv: error loading file.\n");
		exit(1);
	}
	else
	{
	...
	*cdir = 0;
	...
	if (res != 0)
	{
		...
		if (getenv("HOME") == NULL)
			goto badhome;
		chdir(getenv("HOME"));
		if (opendir(&dirfile, ".") != 0)
		{
		badhome:
			screenoff();
			printf("zgv: $HOME is unreadable. This is a Bad Thing. TTFN...\n");
			exit(1);
		}
	...
	entnum = 0;
	while (readdir(&dirfile, &fileInfo) == 0)
	{
		if (!cfg.showxvpicdir || (cfg.showxvpicdir &&
			strcmp(fileInfo.fname, ".xvpics") != 0))
			if (fileInfo.fname[0] == '.' && fileInfo.fname[1] != '.')
				continue;	/* skip (most) 'dot' files */
			/* no `.' ever, and no `..' if at root. */
		if (strcmp(fileInfo.fname, ".") != 0 &&
			!(strcmp(cdir, "/") == 0 && strcmp(fileInfo.fname, "..") == 0))
		{
			if ((stat(fileInfo.fname, &buf)) == -1)
			{
				buf.st_mode = 0;
				buf.st_size = 0;
				buf.st_mtime = 0;
			...
			if (isdir || ispicture(fileInfo.fname))
			{
				entnum++;
				if (!gifdir_resize_if_needed(entnum))
				{
				...
				write_gifdir_name(entnum, fileInfo.fname);
				 ...
				 */
				if ((ptr = strrchr(fileInfo.fname, '.')) == NULL)
					/* use the NUL, Luke */
					gifdir[entnum].extofs = strlen(fileInfo.fname);
				else
					gifdir[entnum].extofs = ptr - fileInfo.fname;
...
int delete_file(char* filename, int doprompt, int report_error)
{
	...
	if (retn == 1)
	{
		...
		snprintf(buf, sizeof(buf), ".xvpics/%s", filename);
		unlink(buf);		/* don't care if it fails */
...
int makedirxv332(char* filename, char* xvpicfn, unsigned int howfar)
{
	...
	int res = opendir(&dirfile, filename);
	if (res != 0) return(_PICERR_NOFILE);

	while (readdir(&dirfile, &info) == 0)
	{
		if (info.fname[0] == '.') continue;		/* skip `dot' files */
		if ((stat(info.fname, &buf)) == -1)
			buf.st_mode = 0;
		if (S_ISDIR(buf.st_mode)) continue;		/* ...and subdirs */
		if (!ispicture(info.fname)) continue;	/* must be picture file */
		...
		pos = -1;
		for (f = 3; f >= 0; f--)
			if (files[f][0] == 0 || strcmp(info.fname, files[f]) < 0)
				pos = f;
		...
		if (pos >= 0)
		{
			...
			strncpy(files[pos], info.fname, sizeof(files[pos]) - 1);
	...
	for (f = 1; f <= gifdirsiz; f++)
	{
			...
			gifdir[f].name[0] != '.')
		{
				...
				do_dirs_instead)
			{
				...
				if (mkdir(".xvpics") != 0 || errno == EEXIST || errno == EACCES)
				{
				...
				usehomedir:
					...
					if (mkdir(buf) != 0 && errno != EEXIST && errno != EACCES)
					{
					...
					if (mkdir(altdirbuf) != 0 && errno != EEXIST && errno != EACCES)
...
int recursive_update_internal(char* dirname, int* firstp)
{
	...
	while ((readdir(&dir, &info)) == 0)
	{
		...
		if ((stat(info.fname, &sbuf)) == -1 || !S_ISDIR(sbuf.st_mode))
			continue;

		/* ok then, recurse. */
		if (!recursive_update_internal(info.fname, firstp))
		{
			aborted = 1;
