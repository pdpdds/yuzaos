...
#else /*linux freebsd*/
#define DEFAULT_CONFIG_FILE "skyos-2.0.conf"
#endif
...
bool Cross::IsPathAbsolute(std::string const& in) {
	// Absolute paths
#if defined (WIN32) || defined(OS2) || defined(SKYOS32)
	// drive letter
...
bool read_directory_first(dir_information* dirp, char* entry_name, bool& is_directory) 
{
	...
	strncpy(entry_name, entry->d_name, CROSS_LEN);
...
YUZA_API void SendSerialLog(const char* fmt, ...)
{
	...
	Syscall_SendSerialLog(buf, strlen(buf) + 1);
}


int GetCommandFromKeyboard(char* buffer, int length)
{
	return Syscall_GetCommandFromKeyboard(buffer, length);
}
...
void exit_process(int errorcode)
{
	Syscall_exit(errorcode);
}
