...
static float noise_normalize(vorbis_look_psy *p, int limit, float *r, float *q, float *f, int *flags, float acc, int i, int n, int *out){
  ...
  vorbis_info_psy *vi=p->vi;
  float **sort = _alloca(n*sizeof(*sort));
  int j,count=0;
...
void _vp_couple_quantize_normalize(int blobno,
  ...
  float **raw = _alloca(ch*sizeof(*raw));

  /* dual pupose; quantized energy (if flag set), othersize fabs(raw) */
  float **quant = _alloca(ch*sizeof(*quant));

  /* floor energy */
  float **floor = _alloca(ch*sizeof(*floor));

  /* flags indicating raw/quantized status of elements in raw vector */
  int   **flag  = _alloca(ch*sizeof(*flag));

  /* non-zero flag working vector */
  int    *nz    = _alloca(ch*sizeof(*nz));

  /* energy surplus/defecit tracking */
  float  *acc   = _alloca((ch+vi->coupling_steps)*sizeof(*acc));

  /* The threshold of a stereo is changed with the size of n */
  if(n > 1000)
    postpoint=stereo_threshholds_limited[g->coupling_postpointamp[blobno]];

  raw[0]   = _alloca(ch*partition*sizeof(**raw));
  quant[0] = _alloca(ch*partition*sizeof(**quant));
  floor[0] = _alloca(ch*partition*sizeof(**floor));
  flag[0]  = _alloca(ch*partition*sizeof(**flag));
