...
HangulProcessor::HangulProcessor()
{	
	m_bHangulMode = false;	
	memset(&stBufferManager, 0, sizeof(BUFFERMANAGER));	
}

void HangulProcessor::Reset()
{
	memset(&stBufferManager, 0, sizeof(BUFFERMANAGER));
}
...
bool HangulProcessor::SwitchMode()
{
...
bool HangulProcessor::InputAscii(unsigned char letter)
{
	switch (letter)
	{
		//------------------------------------------------------------------
...
//------------------------------------------------------------------
int HangulProcessor::GetString(char* buffer)
{		
...
int HangulProcessor::DrawText(int iX1, int iY1, int iX2, int iY2, DWORD* pstMemoryAddress, int iX, int iY,
