...
void GUI_PopupMenu::add(GUI_MenuItem* child) {
    ...
    child->setCallback( [=](GUI_View *v) {
        ...
        for (eastl::vector<GUI_MenuItem *>::iterator it = menuItems.begin() ; it != menuItems.end(); ++it) {
            GUI_MenuItem *c = *it;
            if( lit == c ) {
                c->setSelected(true);
                //GUI_Log( "%s\n", c->title.c_str());
...
void GUI_PopupMenu::remove(GUI_MenuItem* child) {
    ...
    child->setCallback(nullptr);
}

void GUI_PopupMenu::addSimpleMenu( const char *title, bool separator ) {
    GUI_MenuItem *item1 = GUI_MenuItem::create(nullptr, title, 0, 0, -1, 0 );
...
void GUI_MenuBar::remove(GUI_MenuBarItem* child) {
    remove_child(child);
    for (eastl::vector<GUI_MenuBarItem *>::iterator it = menuItems.begin() ; it != menuItems.end(); ++it) {
        if( child == *it ) {
            menuItems.erase( it );
            break;
        }
