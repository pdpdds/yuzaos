...
void TextBox::RefreshCursorBounds()
{
    ...
    m_rectSelectionBounds.x = eastl::min(pA.x, pB.x);
    m_rectSelectionBounds.y = m_text->Y()-1;
    m_rectSelectionBounds.w = eastl::max(pA.x, pB.x)-m_rectSelectionBounds.x;
...
String TextBox::GetSelection()
{
    ...
    int iStart = eastl::min(m_cursorPos, m_cursorEnd);
    int iEnd = eastl::max(m_cursorPos, m_cursorEnd);
