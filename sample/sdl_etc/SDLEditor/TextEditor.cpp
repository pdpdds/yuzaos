...
TextEditor::TextEditor()
	...
	, mStartTime(eastl::chrono::duration_cast<eastl::chrono::milliseconds>(eastl::chrono::system_clock::now().time_since_epoch()).count())
{
...
void TextEditor::SetLanguageDefinition(const LanguageDefinition & aLanguageDef)
{
	...
	for (const LanguageDefinition::TokenRegexString& r : mLanguageDefinition.mTokenRegexStrings)
	{
		mRegexList.push_back(eastl::make_pair(jp::Regex(r.first), r.second));
	}
...
TextEditor::Coordinates TextEditor::SanitizeCoordinates(const Coordinates & aValue) const
{
	...
	{
		column = mLines.empty() ? 0 : eastl::min(column, GetLineMaxColumn(line));
		return Coordinates(line, column);
	}
}
...
void TextEditor::Advance(Coordinates & aCoordinates) const
{
	if (aCoordinates.mLine < (int)mLines.size())
	{
		...
		if (cindex + 1 < (int)line.size())
		{
			...
			cindex = eastl::min(cindex + delta, (int)line.size() - 1);
		}
		else
		{
			++aCoordinates.mLine;
...
TextEditor::Coordinates TextEditor::ScreenPosToCoordinates(const ImVec2& aPosition) const
{
	...
	int lineNo = eastl::max(0, (int)floor(local.y / mCharAdvance.y));
...
TextEditor::Coordinates TextEditor::FindNextWord(const Coordinates & aFrom) const
{
	...
	while (!isword || skip)
	{
		...
		{
			auto l = eastl::max(0, (int) mLines.size() - 1);
...
eastl::string TextEditor::GetWordUnderCursor() const
{
	auto c = GetCursorPosition();
	return GetWordAt(c);
}

eastl::string TextEditor::GetWordAt(const Coordinates & aCoords) const
{
...
void TextEditor::Render()
{
	...
	auto lineMax = eastl::max(0, eastl::min((int)mLines.size() - 1, lineNo + (int)floor((scrollY + contentSize.y) / mCharAdvance.y)));
	...
	if (!mLines.empty())
	{
		...
		while (lineNo <= lineMax)
		{
			...
			longest = eastl::max(mTextStart + TextDistanceToLineStart(Coordinates(lineNo, GetLineMaxColumn(lineNo))), longest);
			auto columnNo = 0;
			...
			for (int i = 0; i < line.size();)
			{
				...
				if (glyph.mChar == '\t')
				{
					...
					bufferOffset.x = (1.0f + floor((1.0f + bufferOffset.x) / (float(mTabSize) * spaceSize))) * (float(mTabSize) * spaceSize);
					++i;
...
void TextEditor::EnterCharacter(ImWchar aChar, bool aShift)
{
	...
	if (aChar == '\n')
	{
		...
		if (mLanguageDefinition.mAutoIndentation)
			for (size_t it = 0; it < line.size() && isascii(line[it].mChar) && isspace(line[it].mChar); ++it)
...
void TextEditor::SetSelectionStart(const Coordinates & aPosition)
{
	...
	if (mState.mSelectionStart > mState.mSelectionEnd)
		eastl::swap(mState.mSelectionStart, mState.mSelectionEnd);
}
...
{
	mTabSize = eastl::max(0, eastl::min(32, aValue));
}

void TextEditor::InsertText(const eastl::string & aValue)
{
...
void TextEditor::InsertText(const char * aValue)
{
	...
	auto start = eastl::min(pos, mState.mSelectionStart);
...
void TextEditor::MoveUp(int aAmount, bool aSelect)
{
	...
	mState.mCursorPosition.mLine = eastl::max(0, mState.mCursorPosition.mLine - aAmount);
...
void TextEditor::MoveDown(int aAmount, bool aSelect)
{
	...
	auto oldPos = mState.mCursorPosition;
	mState.mCursorPosition.mLine = eastl::max(0, eastl::min((int)mLines.size() - 1, mState.mCursorPosition.mLine + aAmount));
...
void TextEditor::MoveRight(int aAmount, bool aSelect, bool aWordMode)
{
	...
	while (aAmount-- > 0)
	{
		...
		if (cindex >= line.size())
		{
			...
			{
				mState.mCursorPosition.mLine = eastl::max(0, eastl::min((int)mLines.size() - 1, mState.mCursorPosition.mLine + 1));
...
eastl::string TextEditor::GetSelectedText() const
{
	return GetText(mState.mSelectionStart, mState.mSelectionEnd);
}

eastl::string TextEditor::GetCurrentLineText()const
{
...
{
	int toLine = aLines == -1 ? (int)mLines.size() : eastl::min((int)mLines.size(), aFromLine + aLines);
	mColorRangeMin = eastl::min(mColorRangeMin, aFromLine);
	mColorRangeMax = eastl::max(mColorRangeMax, toLine);
	mColorRangeMin = eastl::max(0, mColorRangeMin);
	mColorRangeMax = eastl::max(mColorRangeMin, mColorRangeMax);
	mCheckComments = true;
}

static int appendUtf8ToStdStr(eastl::string& buf, char chr) {
	int ret = 0;
	union { char ui; char ch[4]; } u;
	u.ui = chr;
	for (int i = 0; i < 4; ++i) {
		if (u.ch[i]) {
...
static int countUtf8BytesFromStr(const CodePoint* in_text, const CodePoint* in_text_end) {
	int bytes_count = 0;
...
static int countUtf8Bytes(char chr) {
	int ret = 0;
	union { char ui; char ch[4]; } u;
	u.ui = chr;
	for (int i = 0; i < 4; ++i) {
...
void TextEditor::ColorizeRange(int aFromLine, int toLine) {
	if (mLines.empty() || aFromLine >= toLine)
		return;

	eastl::string buffer;
	int endLine = eastl::max(0, eastl::min((int)mLines.size(), toLine));
	for (int i = aFromLine; i < endLine; ++i) {
		bool preproc = false;
		Line& line = mLines[i];
		buffer.clear();
		for (Glyph& g : mLines[i]) {
			appendUtf8ToStdStr(buffer, g.mChar);
...
/*void TextEditor::ColorizeRange(int aFromLine, int aToLine)
{
	...
	eastl::string buffer;
	eastl::cmatch results;
	eastl::string id;

	int endLine = eastl::max(0, eastl::min((int)mLines.size(), aToLine));
	for (int i = aFromLine; i < endLine; ++i)
	{
		auto& line = mLines[i];
		...
		for (auto first = bufferBegin; first != last; )
		{
			...
			if (hasTokenizeResult == false)
			{
				...
				for (auto& p : mRegexList)
				{
					if (eastl::regex_search(first, last, results, p.first, eastl::regex_constants::match_continuous))
					{
						hasTokenizeResult = true;
...
void TextEditor::ColorizeInternal()
{
	...
	if (mColorRangeMin < mColorRangeMax)
	{
		...
		const int to = eastl::min(mColorRangeMin + increment, mColorRangeMax);
		ColorizeRange(mColorRangeMin, to);
		mColorRangeMin = to;

		if (mColorRangeMax == mColorRangeMin)
		{
			mColorRangeMin = eastl::numeric_limits<int>::max();
			mColorRangeMax = 0;
		}
		return;
	}
...
void TextEditor::EnsureCursorVisible()
{
	...
	if (pos.mLine < top)
		ImGui::SetScrollY(eastl::max(0.0f, (pos.mLine - 1) * mCharAdvance.y));
	if (pos.mLine > bottom - 4)
		ImGui::SetScrollY(eastl::max(0.0f, (pos.mLine + 4) * mCharAdvance.y - height));
	if (len + mTextStart < left + 4)
		ImGui::SetScrollX(eastl::max(0.0f, len + mTextStart - 4));
	if (len + mTextStart > right - 4)
		ImGui::SetScrollX(eastl::max(0.0f, len + mTextStart + 4 - width));
}
...
TextEditor::UndoRecord::UndoRecord(
	const eastl::string& aAdded,
	const TextEditor::Coordinates aAddedStart,
	const TextEditor::Coordinates aAddedEnd,
	const eastl::string& aRemoved,
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::CPlusPlus()
{
	...
	if (!inited)
	{
		...
		langDef.mTokenize = [](const char * in_begin, const char * in_end, const char *& out_begin, const char *& out_end, PaletteIndex & paletteIndex) -> bool
		{
			...
			while (in_begin < in_end && isascii(*in_begin) && isspace(*in_begin))
				in_begin++;
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::HLSL()
{
	...
	if (!inited)
	{
		...
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<eastl::string, PaletteIndex>("[ \\t]*#[ \\t]*[a-zA-Z_]+", PaletteIndex::Preprocessor));
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<eastl::string, PaletteIndex>("L?\\\"(\\\\.|[^\\\"])*\\\"", PaletteIndex::String));
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<eastl::string, PaletteIndex>("\\'\\\\?[^\\']\\'", PaletteIndex::CharLiteral));
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<eastl::string, PaletteIndex>("[+-]?([0-9]+([.][0-9]*)?|[.][0-9]+)([eE][+-]?[0-9]+)?[fF]?", PaletteIndex::Number));
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<eastl::string, PaletteIndex>("[+-]?[0-9]+[Uu]?[lL]?[lL]?", PaletteIndex::Number));
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<eastl::string, PaletteIndex>("0[0-7]+[Uu]?[lL]?[lL]?", PaletteIndex::Number));
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<eastl::string, PaletteIndex>("0[xX][0-9a-fA-F]+[uU]?[lL]?[lL]?", PaletteIndex::Number));
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<eastl::string, PaletteIndex>("[a-zA-Z_][a-zA-Z0-9_]*", PaletteIndex::Identifier));
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<eastl::string, PaletteIndex>("[\\[\\]\\{\\}\\!\\%\\^\\&\\*\\(\\)\\-\\+\\=\\~\\|\\<\\>\\?\\/\\;\\,\\.]", PaletteIndex::Punctuation));
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::SQL()
{
	...
	if (!inited)
	{
		...
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<eastl::string, PaletteIndex>("L?\\\"(\\\\.|[^\\\"])*\\\"", PaletteIndex::String));
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<eastl::string, PaletteIndex>("\\\'[^\\\']*\\\'", PaletteIndex::String));
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<eastl::string, PaletteIndex>("[+-]?([0-9]+([.][0-9]*)?|[.][0-9]+)([eE][+-]?[0-9]+)?[fF]?", PaletteIndex::Number));
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<eastl::string, PaletteIndex>("[+-]?[0-9]+[Uu]?[lL]?[lL]?", PaletteIndex::Number));
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<eastl::string, PaletteIndex>("0[0-7]+[Uu]?[lL]?[lL]?", PaletteIndex::Number));
		langDef.mTokenRegexStrings.push_back(eastl::make_pair<eastl::string, PaletteIndex>("0[xX][0-9a-fA-F]+[uU]?[lL]?[lL]?", PaletteIndex::Number));
