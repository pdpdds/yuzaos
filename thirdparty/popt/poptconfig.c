...
int poptReadFile(const char* fn, char** bp, size_t* nbp, int flags)
{
	...
	fseek(fdno, 0, SEEK_END);
	nb = ftell(fdno);
	fseek(fdno, 0, SEEK_SET);

	if ((b = calloc(sizeof(*b), (size_t)nb + 1)) == NULL
		|| fread((char*)b, (size_t)nb, 1, fdno) != (long)nb)
	{
		int oerrno = errno;
		fclose(fdno);
		errno = oerrno;
...
int poptReadDefaultConfig(poptContext con, /*@unused@*/ UNUSED(int useEnv))
{
    ...
    static const char _popt_etc[] = "C:\\etc\\popt";
    char * home;
...
#if defined(HAVE_GLOB_H)
    if (!stat("C:\\etc\\popt.d", &sb) && S_ISDIR(sb.st_mode)) {
	const char ** av = NULL;
	int ac = 0;
	int i;

	if ((rc = poptGlob(con, "C:\\etc\\popt.d\\*", &ac, &av)) == 0) {
	    for (i = 0; rc == 0 && i < ac; i++) {
		const char * fn = av[i];
