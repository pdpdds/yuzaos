...
void ScrollControl::UpdateScrollBars()
{
    ...
    for (auto&& child : m_innerPanel->Children)
    {
        childrenWidth = eastl::max(childrenWidth, child->Right());
        childrenHeight = eastl::max(childrenHeight, child->Bottom());
    }

    if (m_bCanScrollH)
    {
        m_innerPanel->SetSize(eastl::max( Width(), childrenWidth ),
			eastl::max( Height(), childrenHeight ));
    }
    else
    {
