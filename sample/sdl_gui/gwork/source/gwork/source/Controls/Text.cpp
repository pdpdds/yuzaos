...
int Text::GetClosestCharacter(Gwk::Point p)
{
    ...
    for (unsigned i = 0; i < m_string.length()+1; i++)
    {
        ...
        const int iDist = abs(cp.x-p.x) + abs(cp.y-p.y);  // this isn't proper
...
void Text::SplitWords(const Gwk::String& s, eastl::vector<Gwk::String>& elems)
{
    Gwk::String str;
