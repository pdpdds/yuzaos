...
namespace eastl
{
	...
	{
		memmove(pDestination, (void *)pSource, (size_t)(pSourceEnd - pSource) * sizeof(T));
	...
	basic_string<T, Allocator>& basic_string<T, Allocator>::assign(const value_type* pBegin, const value_type* pEnd)
	{
		...
		{
			memmove(mpBegin, (void*)pBegin, (size_t)n * sizeof(value_type));
			erase(mpBegin + n, mpEnd);
		}
		else
		{
		    memmove(mpBegin, (void*)pBegin, (size_t)(mpEnd - mpBegin) * sizeof(value_type));
		...
		if(n)
		{
			...
			if(bCapacityIsSufficient && !bSourceIsFromSelf)
			{
				...
				if(nElementsAfter >= (ptrdiff_t)n) // If the newly inserted characters entirely fit within the size of the original string...
				{
					...
					memmove(const_cast<value_type*>(p), (void*)pBegin, (size_t)(pEnd - pBegin) * sizeof(value_type));
				}
				else
				{
					const value_type* const pMid = pBegin + (nElementsAfter + 1);

					memmove(mpEnd + 1, (void*)pMid, (size_t)(pEnd - pMid) * sizeof(value_type));
					mpEnd += n - nElementsAfter;
						...
						{
					#endif
							memmove(mpEnd, (void*)p, (size_t)(pOldEnd - p + 1) * sizeof(value_type));
							mpEnd += nElementsAfter;
					#if EASTL_EXCEPTIONS_ENABLED
						}
						catch(...)
					...
					memmove(const_cast<value_type*>(p), (void*)pBegin, (size_t)(pMid - pBegin) * sizeof(value_type));
				}
			}
	...
	basic_string<T, Allocator>::erase(const_iterator pBegin, const_iterator pEnd)
	{
		...
		if(pBegin != pEnd)
		{
			memmove(const_cast<value_type*>(pBegin), (void*)pEnd, (size_t)((mpEnd - pEnd) + 1) * sizeof(value_type));
	...
	basic_string<T, Allocator>& basic_string<T, Allocator>::replace(const_iterator pBegin1, const_iterator pEnd1, const value_type* pBegin2, const value_type* pEnd2)
	{
		...
		if(nLength1 >= nLength2) // If we have a non-expanding operation...
		{
			if((pBegin2 > pEnd1) || (pEnd2 <= pBegin1))  // If we have a non-overlapping operation...
				...
				memmove(const_cast<value_type*>(pBegin1), (void*)pBegin2, (size_t)(pEnd2 - pBegin2) * sizeof(value_type));
		...
		else // Else we are expanding.
		{
			if((pBegin2 > pEnd1) || (pEnd2 <= pBegin1)) // If we have a non-overlapping operation...
			{
				...
				if((pEnd2 <= pBegin1) || (pBegin2 > pEnd1))
					...
					memmove(const_cast<value_type*>(pBegin1), (void*)pBegin2, (size_t)(pMid2 - pBegin2) * sizeof(value_type));
