...
#if defined(POSIX) || defined(PLAYSTATION3) || defined(SKYOS32)
...
AbstractFSNode *YuzaFilesystemFactory::makeRootFileNode() const {
	return new YuzaFilesystemNode("/");
}

AbstractFSNode *YuzaFilesystemFactory::makeCurrentDirectoryFileNode() const {
...
#else
	char buf[MAXPATHLEN];
	return getcwd(buf, MAXPATHLEN) ? new YuzaFilesystemNode(buf) : NULL;
#endif
}

AbstractFSNode *YuzaFilesystemFactory::makeFileNodePath(const Common::String &path) const {
	assert(!path.empty());
	return new YuzaFilesystemNode(path);
}
#endif
