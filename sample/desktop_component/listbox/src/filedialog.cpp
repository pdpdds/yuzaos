...
static void reread_dir(void)
{
	...
	if (chdir(hulp) == -1)
	{
		...
		Syscall_GetCurrentDirectory(256, hulp);
	...
	Syscall_GetCurrentDirectory(128, hulp);
	set_input(dir_name, hulp);
	show_input(dir_name);
...
#include <dirent.h>
#if 1
static int select_files(dirent* info)
{
	if (info->d_name[0] == '.')
		return 0;
...
static int create_lists(void)
{
	...
	char buf[256];
	Syscall_GetCurrentDirectory(256, buf);
	absolutePath = buf;
	absolutePath += '\\';
	absolutePath += '.';

	dir_obj = create_directory_list((char*)absolutePath.c_str(), FALSE, dir_list);
	if (dir_obj == NULL)
		return FALSE;
	for (i = 0; i < dir_list->tot_nr_items; i++)
		set_object_callback2(dir_obj[i], reread_dir_cb);

	file_obj = create_directory_list((char*)absolutePath.c_str(), TRUE, file_list);
	if (file_obj == NULL)
		return FALSE;
...
int file_dialog(char* directory, char* filename, char* mask)
{
	...
	strcpy(hulp2, mask);
	line1 = strtok(hulp2, "|");
	while (line1 != NULL && strlen(line1) != 0) {
		line2 = strtok(NULL, "|");
