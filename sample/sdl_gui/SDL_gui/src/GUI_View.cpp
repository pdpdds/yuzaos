...
eastl::vector<GUI_View *>GUI_View::closeQueue;
...
void GUI_View::setUserEventHandler(eastl::function<bool(SDL_Event* ev)>handler ) {
...
void GUI_View::updateLayout() {
    ...
    if (_layout == GUI_LAYOUT_ABSOLUTE) {
        ...
        for (eastl::vector<GUI_View *>::iterator it = children.begin() ; it != children.end(); ++it) {
            GUI_View *child = *it;
