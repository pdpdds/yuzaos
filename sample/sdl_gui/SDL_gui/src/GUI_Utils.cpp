...
eastl::string GUI_GetCurrentPath() {
	//20190618
    ...
    return eastl::string(cCurrentPath);
}

eastl::string GUI_GetResourcePath(const eastl::string &subDir) {
#ifdef _WIN32
    const char PATH_SEP = '\\';
#else
    const char PATH_SEP = '/';
#endif
    static eastl::string baseRes;
    
    baseRes = "";
#if defined (__ANDROID__) || (__MACOSX__) || __SKYOS32__
...
SDL_Texture* GUI_LoadTexture(const eastl::string &filename, SDL_Renderer *ren) {
