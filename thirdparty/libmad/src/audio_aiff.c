 ...
 */
static
void int32__(unsigned char *ptr, unsigned long num)
{
  ptr[0] = num >> 24;
  ptr[1] = num >> 16;
  ptr[2] = num >>  8;
...
int config(struct audio_config *config)
{
  ...
  int32__(&chunk[4],    18);			/* chunkSize */

  int16(&chunk[8],    config->channels);	/* numChannels */
  int32__(&chunk[10],   ~0L);			/* numSampleFrames */
  ...
  int32__(&chunk[4],  ~0L);		/* chunkSize */

  int32__(&chunk[8],  0);			/* offset */
  int32__(&chunk[12], 0);			/* blockSize */
