...
{
  auto iter = eastl::find(begin(textBuffersList_), eastl::end(textBuffersList_), activeTextBuffer_);
...
{
  auto iter = eastl::find(begin(textBuffersList_), eastl::end(textBuffersList_), activeTextBuffer_);
  if (iter == end(textBuffersList_) || iter + 1 == eastl::end(textBuffersList_))
    return;
  eastl::swap(*iter, *(iter + 1));
  update();
}

eastl::vector<BaseTextBuffer *> &Tabs::textBuffersList()
{
  return textBuffersList_;
}

const eastl::vector<BaseTextBuffer *> &Tabs::textBuffersList() const
{
  return textBuffersList_;
}
