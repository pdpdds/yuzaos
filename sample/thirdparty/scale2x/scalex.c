...
int file_process(const char* file0, const char* file1, int opt_scale_x, int opt_scale_y, int opt_crc)
{
	...
	if (scale_precondition(opt_scale_x * 100 + opt_scale_y, pixel, width, height) != 0) {
		printf("Error in the size of the source bitmap. Generally this happen\n");
		printf("when the bitmap is too small or when the width is not an exact\n");
		printf("multiplier of 8 bytes.\n");
		goto err_src;
	}
...
void version(void) {
	printf("scale2x v 3.1 by Andrea Mazzoleni\n");
}
