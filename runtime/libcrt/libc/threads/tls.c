...
thread_storage_t* tls_current(void)
{
    ...
    _In_ tss_t tss_key)
{
    ...
    void *Result            = NULL;
    DataKey_t tKey          = { .Value.Id = Syscall_GetCurrentThreadId() };
    ...
    _In_ void*          DsoHandle)
{
    ...
    printf("tls_register_atexit(%u, 0x%x)\n", ThreadId, DsoHandle);
...
{
	printf("tls_atexit_quick(%u, 0x%x)\n", thr, DsoHandle);
...
tls_cleanup(_In_ thrd_t thr, _In_ void* DsoHandle, _In_ int ExitCode)
{
    ...
    SpinlockAcquire(&TlsLock);
    while (CollectionRemoveByKey(&TlsGlobal.Tls, Key) == 0);
    SpinlockRelease(&TlsLock);
...
{
	printf("tls_cleanup_quick(%u, 0x%x, %i)\n", thr, DsoHandle, ExitCode);
...
{
	return (thrd_t)(Syscall_GetCurrentThreadId());
	...
	tls_current()->thr_id = (thrd_t)(Syscall_GetCurrentThreadId());
	return tls_current()->thr_id;*/
}