...
int vhd_read(struct vhd *vhd,
	     ...
	     size_t size)
{
	if (fseek(vhd->fd, vhd->offset, SEEK_SET) != vhd->offset) {
		fprintf(stderr, "Error: couldn't seek '%s': %s\n",
			vhd->name, strerror(errno));
		return -1;
	}

	if (fread(buf, 1, size, vhd->fd) !=  (int) size) {
	      ...
	      size_t size)
{
	...
	if (fwrite(buf, 1, size, vhd->fd) !=  (int) size) {
		fprintf(stderr, "Error: couldn't read from '%s': %s\n",
			vhd->name, strerror(errno));
		return -1;
	}
	     ...
	     unsigned flags)
{
	...
	if (flags & OPEN_CREAT) {
		vhd->fd = fopen(vhd->name, "rw");
	} else {
		vhd->fd = fopen(vhd->name, flags & OPEN_RW ?
			       "rw" : "rb");
	}
	if (vhd->fd == 0) {
...
int vhd_close(struct vhd *vhd, int status)
{
	if (vhd->fd != 0) {
		if (!status) {
			if (fsync(vhd->fd)) {
				printf("couldn't flush VHD data\n");
				return -1;
			}

			if (fclose(vhd->fd)) {
				printf("couldn't close VHD file\n");
				return -1;
			}
		} else {
			if (vhd->flags & OPEN_CREAT) {
				if (unlink(vhd->name)) {
					printf("couldn't clean up VHD file\n");
					return -1;
				}
			}
		}
...
int vhdtool_main(int argc, char **argv)
{
	int status;
	off64_t vhd_size = 0;
	uint32_t vhd_type = 0;
