...
{
  auto iter = eastl::find(begin(layoutablesList_), end(layoutablesList_), value);
...
static eastl::vector<int> calculateDimensions(eastl::vector<eastl::pair<int, int>> minMax, int dimension)
{
    eastl::vector<int> result;
  eastl::vector<bool> untouchableList;
...
void Layout::resize(int width, int height)
{
  ...
  if (style_ == Vertical)
  {
      eastl::vector<eastl::pair<int, int> > minMax;
    for (auto &l: layoutablesList_)
      minMax.push_back(eastl::make_pair(l->minHeight(), l->maxHeight()));
  ...
  else // if style == Horizontal
  {
      eastl::vector<eastl::pair<int, int> > minMax;
    for (auto &l: layoutablesList_)
      minMax.push_back(eastl::make_pair(l->minWidth(), l->maxWidth()));
...
eastl::vector<Layoutable *> Layout::children() const
{
