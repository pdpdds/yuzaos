...
{
    m_selectedRows.erase(eastl::find(m_selectedRows.begin(), m_selectedRows.end(), row));
    m_table->Remove(row);
}
...
{
	eastl::list<Layout::TableRow*>::iterator it = m_selectedRows.begin();
...
bool ListBox::OnKeyDown(bool bDown)
{
    if (bDown)
    {
        ...
        Base::List::const_iterator result = eastl::find(begin, end, sel_row);
