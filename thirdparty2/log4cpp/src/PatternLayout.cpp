...
namespace log4cpp {
    ...
    struct CategoryNameComponent : public PatternLayout::PatternComponent {
        ...
        virtual void append(std::ostringstream& out, const LoggingEvent& event) {
            if (_precision == -1) {
                ...
                out << event.categoryName.substr(begin).c_str();
            }
        }
    ...
    struct PriorityComponent : public PatternLayout::PatternComponent {
        virtual void append(std::ostringstream& out, const LoggingEvent& event) {
            out << Priority::getPriorityName(event.priority).c_str();
        }
    };

    struct ThreadNameComponent : public PatternLayout::PatternComponent {
        virtual void append(std::ostringstream& out, const LoggingEvent& event) {
            out << event.threadName.c_str();
        }
    };
    ...
    struct FormatModifierComponent : public PatternLayout::PatternComponent {
        ...
        virtual void append(std::ostringstream& out, const LoggingEvent& event) {
            ...
            if (_minWidth > msg.length()) {
                if (_alignLeft) {
                    out << msg.c_str() << std::string(fillCount, ' ').c_str();
                } else {
                    out << std::string(fillCount, ' ').c_str() << msg.c_str();
                }
            } else {
                out << msg.c_str();
            }
    ...
    void PatternLayout::setConversionPattern(const std::string& conversionPattern) {
...
#else
        std::istream conversionStream(conversionPattern.c_str());
#endif
        std::string literal;
		...
		//20200110
        /*while (conversionStream.operator>>(ch)) {
            if (ch == '%') {
                // readPrefix;
                {
                    char ch2;
                ...
                if (!conversionStream.get(ch)) {
                    ...
                    msg << "unterminated conversion specifier in '" << conversionPattern.c_str() << "' at index " << conversionStream.tellg();
                ...
                default:
                    std::ostringstream msg;
                    msg << "unknown conversion specifier '" << ch << "' in '" << conversionPattern.c_str() << "' at index " << conversionStream.tellg();
                    throw ConfigureFailure(msg.str());                    
                }
                if (component) {
                    if (!literal.empty()) {
