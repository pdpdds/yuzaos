...
const char replay_magic_number[4] = "P1R";
...
int read_replay_header(replay_header_type* header, FILE* fp, char* error_message) {
	...
	word classa;
	fread(&classa, sizeof(classa), 1, fp);
	...
	if (classa != replay_format_class) {
		...
		if (error_message != NULL) {
			snprintf(error_message, REPLAY_HEADER_ERROR_MESSAGE_MAX,
			         ...
			         header->implementation_name, replay_format_class, classa);
		}
		return 0;
	}
	...
	if (is_validate_mode) {
		...
		if (!is_replay_info_printed) {
			...
			printf("Format: class identifier %d, version number %d, deprecation number %d.\n",
				classa, version_number, deprecation_number);
...
void list_replay_files() {
	...
	do {
		...
		if (num_replay_files > max_replay_files) {
			...
			replay_list = krealloc( replay_list, max_replay_files * sizeof( replay_info_type ) );
		}
