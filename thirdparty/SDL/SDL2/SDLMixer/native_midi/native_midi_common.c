...
static MIDIEvent *CreateEvent_Mixer(Uint32 time, Uint8 event, Uint8 a, Uint8 b)
{
    MIDIEvent *newEvent;
...
static MIDIEvent *MIDITracktoStream(MIDITrack *track)
{
    ...
    int currentPos = 0;
    int end = 0;
    MIDIEvent *head = CreateEvent_Mixer(0,0,0,0); /* dummy event to make handling the list easier */
    ...
    while (!end)
    {
        ...
        if (((event>>4) & 0x0F) == MIDI_STATUS_SYSEX)
        {
            ...
            currentEvent->next = CreateEvent_Mixer(atime, event, type, 0);
        ...
        else
        {
            ...
            switch(laststatus)
            {
                ...
                case MIDI_STATUS_PITCH_WHEEL: /* Pitch wheel */
                    ...
                    currentEvent->next = CreateEvent_Mixer(atime, (Uint8)((laststatus<<4)+lastchan), a, b);
                    currentEvent = currentEvent->next;
                    if (NULL == currentEvent)
                    {
                        FreeMIDIEventList(head);
                ...
                case MIDI_STATUS_PRESSURE: /* Channel pressure */
                    a &= 0x7f;
                    currentEvent->next = CreateEvent_Mixer(atime, (Uint8)((laststatus<<4)+lastchan), a, 0);
                    currentEvent = currentEvent->next;
                    if (NULL == currentEvent)
                    {
                        FreeMIDIEventList(head);
