...
void Screen::setCursor(Coord value)
{
  ...
  if (value.y > eastl::max(0, vScroll_ + heightCh() - 2))
  {
...
eastl::wstring Screen::getSelected() const
{
  if (startSelection().y == endSelection().y)
  {
    auto s = eastl::min(startSelection().x, endSelection().x);
    auto e = eastl::max(startSelection().x, endSelection().x);
    const auto &line = (*textBuffer_)[startSelection().y];
    eastl::wstring tmp{ begin(line) + s, begin(line) + e };
    return tmp;
  }
  else
  {
...
void Screen::startIsearch()
{
  if (!statusBar_)
  {
    if (auto *buffer = textBuffer_->As<IsearchBuffer>())
      buffer->findNext();
  }
  else
  {
