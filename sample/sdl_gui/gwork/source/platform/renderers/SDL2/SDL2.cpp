...
Font::Status SDL2::LoadFont(const Font& font)
{
    ...
    if (tfont)
    {
        ...
        CustomFontDeleter customFontDeleter;
        fontData.tFont = unique_ptr<TTF_Font, CustomFontDeleter>(tfont, customFontDeleter);
        m_lastFont = &(*m_fonts.insert(eastl::make_pair(font, eastl::move(fontData))).first);
...
Texture::Status SDL2::LoadTexture(const Texture& texture)
{
    ...
    if (texture.readable)
    {
        ...
        else
        {
            ...
            CustomSurfaceDeleter customSurfaceDeleter;
            texData.surface = unique_ptr<SDL_Surface, CustomSurfaceDeleter>(surf, customSurfaceDeleter);
    ...
    if (tex)
    {
        ...
        CustomTextureDeleter customTextureDeleter;
        texData.texture = unique_ptr<SDL_Texture, CustomTextureDeleter>(tex, customTextureDeleter);
         texData.width = w;
        texData.height = h;
        m_lastTexture = &(*m_textures.insert(eastl::make_pair(texture, eastl::move(texData))).first);
