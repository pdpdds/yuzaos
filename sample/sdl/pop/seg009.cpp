...
void find_exe_dir() {
	...
	strncpy(exe_dir, "./prince/", sizeof(exe_dir));
...
static SDL_RWops* open_dat_from_root_or_data_dir(const char* filename) {
	SDL_RWops* fp = NULL;
	...
	if (fp == NULL) 
	{
		...
		fp = SDL_RWFromFile(data_path, "rb");
...
dat_type *__pascal open_dat(const char *filename, int drive) {
	...
	if (!use_custom_levelset) {
		...
		if (!skip_mod_data_files) {
			...
			fp = SDL_RWFromFile(filename_mod, "rb");
		}
	...
	if (fp != NULL) {
		if (SDL_RWread(fp, &dat_header, 6, 1) != 1)
			goto failed;
		dat_table = (dat_table_type*)malloc(dat_header.table_size);
		if (dat_table == NULL ||
			SDL_RWseek(fp, dat_header.table_offset, SEEK_SET) == -1 ||
			SDL_RWread(fp, dat_table, dat_header.table_size, 1) == 0)
			goto failed;
		pointer->handle = fp;
...
extern "C" byte hc_small_font_data[];
...
void ogg_callback(void *userdata, Uint8 *stream, int len) {
	...
	if (is_sound_on) {
		...
		byte* discarded_samples = (byte*)malloc(len);
		samples_filled = stb_vorbis_get_samples_short_interleaved(ogg_decoder, output_channels,
...
void load_sound_names() {
	const char* names_path = "./prince/data/music/names.txt";
	if (sound_names != NULL) return;
	SDL_RWops* fp = SDL_RWFromFile(names_path, "rt");
	if (fp == NULL)
		return;
	sound_names = (char**)calloc(sizeof(char*) * max_sound_id, 1);

	size_t file_length = SDL_RWseek(fp, 0, SEEK_END);
	char* destination = (char*)malloc(file_length + 1);
	// Reset seek to beginning of file and read text
	SDL_RWseek(fp, 0, SEEK_SET);
	int n_blocks = SDL_RWread(fp, (destination), file_length, 1);
	if (n_blocks <= 0)
		return;
...
#ifdef _WIN32
	char * line = strtok(strdup(destination), "\n");
#else
	char * line = strtok(strdup(destination), "\n");
#endif // _WIN32    
	while (line) {
...
sound_buffer_type* load_sound(int index) {
		...
		if (sound_names != NULL && sound_name(index) != NULL) {
			...
			do {
				...
				if (fp == NULL && !skip_normal_data_files) {
					...
					fp = SDL_RWFromFile(locate_file(filename), "rb");
				}
				if (fp == NULL) {
					break;
				}
				...
				size_t file_size = (size_t)MAX(0, SDL_RWsize(fp));
				byte* file_contents = (byte*)malloc(file_size);
				if (SDL_RWread(fp, file_contents, 1, file_size) != file_size) {
					free(file_contents);
					SDL_RWclose(fp);
					break;
				}
	...
	{
		char* soundName = sound_name(index);
		if(soundName)
...
#if defined(_WIN32) || defined(SKYOS32)
			pop_window_width, pop_window_height, flags);
#else
		pop_window_width, pop_window_height, SDL_WINDOW_FULLSCREEN);
#endif
...
void draw_overlay() {
	...
	if (overlay != 0) {
		...
		if (overlay == 1) {
			...
			int extra_numeric_chars = MAX(0, strlen(timer_text));
...
void load_from_opendats_metadata(int resource_id, const char* extension, SDL_RWops** out_fp, data_location* result, byte* checksum, int* size, dat_type** out_pointer) {
		...
		if (pointer->handle != NULL) {
			...
			if (i < dat_table->res_count) {
				...
				*size = dat_table->entries[i].size;
				if (SDL_RWseek(fp, dat_table->entries[i].offset, SEEK_SET) == -1 ||
					SDL_RWread(fp, checksum, 1, 1) == 0) {
			...
			snprintf(image_filename, sizeof(image_filename), "prince/data/%s/res%d.%s", filename_no_ext, resource_id, extension);
			if (!use_custom_levelset) {
				//printf("loading (binary) %s",image_filename);
				fp = SDL_RWFromFile(image_filename, "rb");
			}
			else {
				...
				fp = SDL_RWFromFile(image_filename_mod, "rb");
				if (fp == NULL) {
					fp = SDL_RWFromFile(image_filename, "rb");
				}
			}
...
void __pascal far close_dat(dat_type far *pointer) {
	...
	while (curr != NULL) {
		if (curr == pointer) {
			*prev = curr->next_dat;
			if (curr->handle) SDL_RWclose(curr->handle);
...
void far *__pascal load_from_opendats_alloc(int resource, const char* extension, data_location* out_result, int* out_size) {
	...
	//read(fd, area, size);
	if (SDL_RWread(fp, area, size, 1) == 0) {
		printf("%s: %s, resource %d, size %d, failed: %d\n", __func__, pointer->filename, resource,
			size, errno);
		free(area);
		area = NULL;
	}
	if (result == data_directory) SDL_RWclose(fp);
	/* XXX: check checksum */
	return area;
}
...
int __pascal far load_from_opendats_to_area(int resource, void far *area, int length, const char* extension) {
	...
	if (SDL_RWread(fp, area, MIN(size, length), 1) == 0) {
		printf("%s: %s, resource %d, size %d, failed: %d\n", __func__, pointer->filename, resource, size, errno);
...
void process_events() {
	...
	while (SDL_PollEvent(&event) == 1) { // while there are still events to be processed
		...
		case SDL_KEYUP:
			...
			else if (is_menu_shown && event.key.keysym.scancode == SDL_SCANCODE_LSHIFT) {
				pressed_enter = true;
				break;
			}
