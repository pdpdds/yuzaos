...
// =============================================================================
{
	...
	retval->n     = (char*)strrchr(retval->name, 'n'); // for transposing macro's
...
static ABCHANDLE *ABC_Init(void)
{
	...
	if( p ) {
		...
		srandom((uint32_t)Syscall_GetTickCount());	// initialize random generator with seed
...
BOOL CSoundFile::ReadABC(const uint8_t *lpStream, DWORD dwMemLength)
{
	...
	mm.sz = dwMemLength;
	mm.pos = 0;
	while( avoid_reentry ) Syscall_Sleep(1);
	avoid_reentry = 1;
	pat_resetsmp();
	pat_init_patnames();
	m_nDefaultTempo = 0;
