...
void P_LoadVertexes (int lump)
{
  ...
  for (i=0; i<numvertexes; i++)
    {
      vertexes[i].x = SHORT_(((mapvertex_t *) data)[i].x)<<FRACBITS;
      vertexes[i].y = SHORT_(((mapvertex_t *) data)[i].y)<<FRACBITS;
    }
  ...
  for (i=0; i<numsegs; i++)
    {
      ...
      li->v1 = &vertexes[SHORT_(ml->v1)];
      li->v2 = &vertexes[SHORT_(ml->v2)];

      li->angle = (SHORT_(ml->angle))<<16;
      li->offset = (SHORT_(ml->offset))<<16;
      linedef = SHORT_(ml->linedef);
      ldef = &lines[linedef];
      li->linedef = ldef;
      side = SHORT_(ml->side);
...
void P_LoadSubsectors (int lump)
{
  ...
  for (i=0; i<numsubsectors; i++)
    {
      subsectors[i].numlines  = SHORT_(((mapsubsector_t *) data)[i].numsegs );
      subsectors[i].firstline = SHORT_(((mapsubsector_t *) data)[i].firstseg);
    }
  ...
  for (i=0; i<numsectors; i++)
    {
      ...
      ss->floorheight = SHORT_(ms->floorheight)<<FRACBITS;
      ss->ceilingheight = SHORT_(ms->ceilingheight)<<FRACBITS;
      ss->floorpic = R_FlatNumForName(ms->floorpic);
      ss->ceilingpic = R_FlatNumForName(ms->ceilingpic);
      ss->lightlevel = SHORT_(ms->lightlevel);
      ss->special = SHORT_(ms->special);
      ss->oldspecial = SHORT_(ms->special);
      ss->tag = SHORT_(ms->tag);
      ss->thinglist = NULL;
  ...
  for (i=0; i<numnodes; i++)
    {
      ...
      no->x = SHORT_(mn->x)<<FRACBITS;
      no->y = SHORT_(mn->y)<<FRACBITS;
      no->dx = SHORT_(mn->dx)<<FRACBITS;
      no->dy = SHORT_(mn->dy)<<FRACBITS;

      for (j=0 ; j<2 ; j++)
        {
          int k;
          no->children[j] = SHORT_(mn->children[j]);
          for (k=0 ; k<4 ; k++)
            no->bbox[j][k] = SHORT_(mn->bbox[j][k])<<FRACBITS;
        }
    }
  ...
  for (i=0; i<numthings; i++)
    {
      ...
      mt->x = SHORT_(mt->x);
      mt->y = SHORT_(mt->y);
      mt->angle = SHORT_(mt->angle);
      mt->type = SHORT_(mt->type);
      mt->options = SHORT_(mt->options);
  ...
  for (i=0; i<numlines; i++)
    {
      ...
      ld->flags = SHORT_(mld->flags);
      ld->special = SHORT_(mld->special);
      ld->tag = SHORT_(mld->tag);
      v1 = ld->v1 = &vertexes[SHORT_(mld->v1)];
      v2 = ld->v2 = &vertexes[SHORT_(mld->v2)];
      ld->dx = v2->x - v1->x;
      ld->dy = v2->y - v1->y;
      ...
      ld->sidenum[0] = SHORT_(mld->sidenum[0]);
      ld->sidenum[1] = SHORT_(mld->sidenum[1]);
  ...
  for (i=0; i<numsides; i++)
    {
      ...
      sd->textureoffset = SHORT_(msd->textureoffset)<<FRACBITS;
      sd->rowoffset = SHORT_(msd->rowoffset)<<FRACBITS;

      // killough 4/4/98: allow sidedef texture names to be overloaded
      // killough 4/11/98: refined to allow colormaps to work as wall
      // textures if invalid as colormaps but valid as textures.

      sd->sector = sec = &sectors[SHORT_(msd->sector)];
      switch (sd->special)
        {
...
void P_LoadBlockMap (int lump)
{
  ...
  if (M_CheckParm("-blockmap") || (count = W_LumpLength(lump)/2) >= 0x10000)
  ...
  else
    {
      ...
      blockmaplump[0] = SHORT_(wadblockmaplump[0]);
      blockmaplump[1] = SHORT_(wadblockmaplump[1]);
      blockmaplump[2] = (long)(SHORT_(wadblockmaplump[2])) & 0xffff;
      blockmaplump[3] = (long)(SHORT_(wadblockmaplump[3])) & 0xffff;

      for (i=4 ; i<count ; i++)
        {
          short t = SHORT_(wadblockmaplump[i]);          // killough 3/1/98
