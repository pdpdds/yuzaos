...
static void R_GenerateComposite(int texnum)
{
	...
	for (; --i >= 0; patch++)
	{
		...
		int x, x1 = patch->originx, x2 = x1 + SHORT_(realpatch->width);
		...
		for (x = x1; x < x2; x++)
			if (collump[x] == -1)      // Column has multiple patches?
			  // killough 1/25/98, 4/9/98: Fix medusa bug.
				R_DrawColumnInCache((column_t*)((byte*)realpatch + LONG_(cofs[x])),
	...
	if (texture->patchcount > 1 && texture->height < 256)
	{
		...
		for (i = texture->patchcount, patch = texture->patches; --i >= 0;)
		{
			...
			for (x = x1; x < x2; x++)
				...
				{
					const column_t* col =
						(column_t*)((byte*)realpatch + LONG_(cofs[x]));
	...
	for (i = 0; i < numtextures; i++, directory++)
	{
		...
		texture = textures[i] =
			Z_Malloc(sizeof(texture_t) +
				sizeof(texpatch_t) * (SHORT_(mtexture->patchcount) - 1),
				PU_STATIC, 0);

		texture->width = SHORT_(mtexture->width);
		texture->height = SHORT_(mtexture->height);
		texture->patchcount = SHORT_(mtexture->patchcount);
		...
		{
			patch->originx = SHORT_(mpatch->originx);
			patch->originy = SHORT_(mpatch->originy);
			patch->patch = patchlookup[SHORT_(mpatch->patch)];
			if (patch->patch == -1)
			{	      // killough 8/8/98
				printf("\nR_InitTextures: Missing patch %d in texture %.8s",
					SHORT_(mpatch->patch), texture->name); // killough 4/17/98
				++errors;
			}
		}
...
void R_InitSpriteLumps(void)
{
	...
	for (i = 0; i < numspritelumps; i++)
	{
		...
		spritewidth[i] = SHORT_(patch->width) << FRACBITS;
		spriteoffset[i] = SHORT_(patch->leftoffset) << FRACBITS;
		spritetopoffset[i] = SHORT_(patch->topoffset) << FRACBITS;
	}
}
