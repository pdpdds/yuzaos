...
vorbis_info_residue *res0_unpack(vorbis_info *vi, oggpack_buffer *opb) {
	int j, acc = 0;
	vorbis_info_residue0 *info = _ogg_calloc(1 * sizeof(*info), 1);
...
vorbis_look_residue *res0_look(vorbis_dsp_state *vd,
	...
	vorbis_info_residue0 *info = (vorbis_info_residue0 *)vr;
	vorbis_look_residue0 *look = _ogg_calloc(1 * sizeof(*look), 1);
	...
	look->partbooks = _ogg_calloc(look->parts * sizeof(*look->partbooks), 1);

	for (j = 0; j < look->parts; j++) {
		int stages = ilog(info->secondstages[j]);
		if (stages) {
			if (stages > maxstage)maxstage = stages;
			look->partbooks[j] = _ogg_calloc(stages * sizeof(*look->partbooks[j]), 1);
			for (k = 0; k < stages; k++)
	...
	for (j = 0; j < look->partvals; j++) {
		...
		memset(look->decodemap[j], 0, dim * sizeof(*look->decodemap[j]));
		for (k = 0; k < dim; k++) {
			long deco = val / mult;
			val -= deco*mult;
			mult /= look->parts;
