...
void check_double_click(GuiObject *obj)
{
	...
	DWORD curTick = Syscall_GetTickCount();
	while (GuiMouseGetButton() != EVENT_MOUSE_LBUTTONDOWN) {
		//Syscall_Sleep(sleep_time);
		do_window_functions(win_thread);
		if (GuiGetMessage() == EVENT_MOUSE_LBUTTONDOWN)
			move_mouse();
	}
	DWORD LastTick = Syscall_GetTickCount();

	if (LastTick - curTick < 100) {		/* Wait for the mousebutton to be released */
		while (GuiMouseGetButton() == EVENT_MOUSE_LBUTTONDOWN) {
			//Syscall_Sleep(sleep_time);
			do_window_functions(win_thread);
			if (GuiGetMessage() == EVENT_MOUSE_LBUTTONDOWN)
