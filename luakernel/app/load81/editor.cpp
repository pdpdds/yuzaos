...
void editorRowAppendString(erow *row, char *s) {
    ...
    row->chars = (char*)realloc(row->chars,row->size+l+1);
...
char *editorTemplate[] = {
    ...
    "    background(0,0,0)",
    "    fill(200,200,200,1)",
    "    text(0,0,\"Press ESC to edit this program.\")",
    "end",
    NULL
};
...
int editorEvents(void) {
    ...
    while (SDL_PollEvent(&event)) {
        ...
        case SDL_MOUSEBUTTONDOWN:
            editorMouseClicked(event.button.x, E.fb->height-1-event.button.y,
                               event.button.button);
            break;
        case SDL_QUIT:
            Syscall_exit(0);
