								  ...
								  DWORD		&resultAddress)
{
	...
	if (useAlternateLoadAddress)
	{
		...
		offset = address - alternateLoadAddress;
		...
		offset = address - preferredLoadAddress;
	...
	if (matchingSymbol != NULL)
	{
		...
		{
			resultAddress;
			int diff = alternateLoadAddress - preferredLoadAddress;
	...
	for(i = 0; i < numSymbols; i++)
	{
		...
		if (mfs != NULL)
		{
			...
			if (partialMatch)
			{
				...
				ptr = strstr(s.c_str(), symbolName.c_str());
				if (ptr != NULL)
				{
					symbol = mfs;
					break;
			...
			else
			{
				...
				if (strcmp(symbolName.c_str(), s.c_str()) == 0)
				{
					symbol = mfs;
					break;
				}
...
int MapFileReader::readSectionInformation(FILE	*fp)
{
	...
	if (strcmp(wStart.c_str(), "Start") != 0)
		return FALSE;
	if (strcmp(wLength.c_str(), "Length") != 0)
		return FALSE;
	if (strcmp(wName.c_str(), "Name") != 0)
		return FALSE;
	if (strcmp(wClass.c_str(), "Class") != 0)
		return FALSE;
	...
	while(TRUE)
	{
		...
		mfa = new MapFileAddress(wStart.c_str());
		mfs->setAddress(mfa);

		length = (DWORD)strtoul(wLength.c_str(), NULL, 16);
...
int MapFileReader::readPublicSymbols(FILE	*fp)
{
	...
	if (strcmp(wAddress.c_str(), "Address") != 0)
		return FALSE;
	if (strcmp(wPublics.c_str(), "Publics") != 0)
		return FALSE;
	if (strcmp(wBy.c_str(), "by") != 0)
		return FALSE;
	if (strcmp(wValue.c_str(), "Value") != 0)
		return FALSE;
	if (strcmp(wRvaBase.c_str(), "Rva+Base") != 0)
		return FALSE;
	if (strcmp(wLibObject.c_str(), "Lib:Object") != 0)
		return FALSE;
							   ...
							   FILE		*fp)
{
	...
	p = getNextWord(p, wName);
	if (strcmp(wOrdinal.c_str(), "ordinal") != 0)
		return FALSE;
	if (strcmp(wName.c_str(), "name") != 0)
		return FALSE;
	...
	do
	{
		...
		p = getNextWord(p, wO);
		ordinal = (DWORD)strtoul(wO.c_str(), NULL, 10);	// I think its base 10!
		p = getNextWord(p, wN1);
									 ...
									 FILE	*fp)
{
	...
	if (strcmp(wStatic.c_str(), "Static") != 0)
		return FALSE;
	if (strcmp(wSymbols.c_str(), "symbols") != 0)
		return FALSE;
