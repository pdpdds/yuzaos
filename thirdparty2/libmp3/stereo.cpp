	...
	if (joint_stereo_mode & 0x1) //intensity stereo processing
	{
		...
		{ 
			...
			if (data.mixed_block[gr][0])
			{
				...
				for (sfb = 0; sfb < 8; sfb++) if (kScaleFactorBandIndices[sfreq].scalel[sfb] >= data.count1[gr][1]) StereoIntensityLong(sfreq, data, gr, sfb, is);
		...
		else 
		{
			...
			for (UInt sfb = 0; sfb < 21; sfb++) if (kScaleFactorBandIndices[sfreq].scalel[sfb] >= data.count1[gr][1]) StereoIntensityLong(sfreq, data, gr, sfb, is);
		}
	}
}
...
void OpenMP3::StereoIntensityLong(UInt sfreq, const FrameData & data, unsigned gr, unsigned sfb, Float32 is[2][576])
{
	...
	if ((is_pos = data.scalefac_l[gr][0][sfb]) != 7) 
	{
		...
		sfb_start = kScaleFactorBandIndices[sfreq].scalel[sfb];
		
		sfb_stop = kScaleFactorBandIndices[sfreq].scalel[sfb + 1];
