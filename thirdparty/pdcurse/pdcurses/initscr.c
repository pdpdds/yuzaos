...
WINDOW *Xinitscr(int argc, char *argv[])
{
    ...
    {
        printf("initscr(): Unable to create SP\n");
        exit(8);
    }
    ...
    if (LINES < 2 || COLS < 2)
    {
        printf( "initscr(): LINES=%d COLS=%d: too small.\n",
                LINES, COLS);
        exit(4);
    }
    ...
    if (!curscr)
    {
        printf("initscr(): Unable to create curscr.\n");
        exit(2);
    }
    ...
    if (!SP->lastscr)
    {
        printf("initscr(): Unable to create SP->lastscr.\n");
        exit(2);
    }
    ...
    if (!stdscr)
    {
        printf("initscr(): Unable to create stdscr.\n");
        exit(1);
    }
