...
static GuiObject *pulldown_loop(GuiObject * obj)
{
	...
	if (GuiMouseGetButton() == EVENT_MOUSE_LBUTTONDOWN)
		mouse_hold = TRUE;
	...
	while (!exit_loop) {
		//ksleep(sleep_time);
		do_window_functions(win_thread);
		int message = GuiGetMessage();
		if (message == EVENT_MOUSE_LBUTTONDOWN || message == EVENT_MOUSE_LBUTTONUP) {
			move_mouse();
			object_region(win);
			...
			if ((status == EVENT_MOUSE_LBUTTONDOWN && !mouse_hold) ||
			    (status != EVENT_MOUSE_LBUTTONDOWN && mouse_hold))
				exit_loop = TRUE;
		}
	}

	/* wait for mousebutton to be released */
	while (status == EVENT_MOUSE_LBUTTONDOWN && !next_pulldown && !next_item) {
		//ksleep(sleep_time);
		do_window_functions(win_thread);
		if (GuiGetMessage() == EVENT_MOUSE_LBUTTONDOWN)
			status = GuiMouseGetButton();
