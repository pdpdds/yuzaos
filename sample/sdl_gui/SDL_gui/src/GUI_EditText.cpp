...
GUI_EditText *GUI_EditText::create( GUI_View *parent, const char *title, int x, int y, int width, int height,
	eastl::function<void(GUI_View*)>callbackFunction ) {
...
GUI_EditText::GUI_EditText(GUI_View *parent, const char *title, int x, int y, int width, int height,
	eastl::function<void(GUI_View*)>callbackFunction ) :
GUI_TextView(parent, title, GUI_GetUITextFontName().c_str(), GUI_GetUITextFontSize(), x, y, width, height, nullptr)
{
    dragable = false;
    clickable = true;
    focusable = true;
...
bool GUI_EditText::eventHandler(SDL_Event*event) {
    ...
    switch (event->type) {
        ...
        case SDL_TEXTINPUT:
        {
            ...
            title = title.insert(textEditIndex, eastl::string(event->text.text));
...
void GUI_EditText::textSelectionCopy() {
    if (GUI_lastEditTextView == this) {
        if (textSelectionIsSelected()) {
            ...
            int endIndex = 0;
            textSelectionGetSelectedIndex(&startIndex, &endIndex);
			eastl::string str = title.substr(startIndex, endIndex - startIndex);
...
void GUI_EditText::textSelectionPaste() {
    if (GUI_lastEditTextView == this) {
        ...
        if (textSelectionIsValidClipboardText()) {
            ...
            title = title.insert(textEditIndex, eastl::string(text));
...
bool GUI_EditText::textSelectionIsThaiText(eastl::string text) {
    bool isThai = false;
...
bool GUI_EditText::textSelectionIsDigitText(eastl::string strText) {
    bool result = true;
