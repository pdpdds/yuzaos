...
Int as_int(float v)
{
	return static_cast<Int>(floor(v * kFixedBias));
}
...
inline uint8_t sample_texture(const Texture& texture, const ImVec2& uv)
{
	...
	tx = max(tx, 0);
	tx = min(tx, texture.width - 1);
	ty = max(ty, 0);
	ty = min(ty, texture.height - 1);
	...
	Stats*             stats)
{
	...
	max_x_i = min(max_x_i, target.width);
	max_y_i = min(max_y_i, target.height);
	...
	Stats*             stats)
{
	...
	min_x_f = max(min_x_f, target.scale.x * clip_rect.x);
	min_y_f = max(min_y_f, target.scale.y * clip_rect.y);
	max_x_f = min(max_x_f, target.scale.x * clip_rect.z - 0.5f);
	max_y_f = min(max_y_f, target.scale.y * clip_rect.w - 0.5f);
