...
Goal_Think::Goal_Think(Player* pBot):Goal_Composite<Player>(pBot, goal_think)
{
  ...
  double BombBias = RandInRange(LowRangeOfBias, HighRangeOfBias);
  ...
  m_Evaluators.push_back(new GetBombGoal_Evaluator(BombBias));
  m_Evaluators.push_back(new ExploreGoal_Evaluator(ExploreBias));
  m_Evaluators.push_back(new InstallBombGoal_Evaluator(BombBias));
...
void Goal_Think::AddGoal_InstallBomb()
{
	if (notPresent(goal_install_bomb))
	{
		RemoveAllSubgoals();
		AddSubgoal(new Goal_InstallBomb(m_pOwner));
	}
}
...
{
  std::list<Goal<Player>*>::iterator curG;
