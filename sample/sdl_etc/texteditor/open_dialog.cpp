...
void OpenDialog::postInsert(Coord& cursor, const eastl ::wstring& value)
{
...
void OpenDialog::scanDirectory()
{
  ...
  setName(eastl::wstring{begin(currentDir) + currentDir.rfind(L'/') + 1, end(currentDir)});
  ...
  closedir(d);
  eastl::sort(eastl::begin(buffer_) + 1, eastl::end(buffer_));
}
