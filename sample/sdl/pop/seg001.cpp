...
void __pascal far hof_write() {
	...
	// no O_TRUNC
	fp = fopen(hof_path, "rw");
	if (fp == nullptr ||
		fwrite(&hof_count, 2, 1, fp) != 2 ||
		fwrite(&hof, sizeof(hof), 1, fp) != sizeof(hof) ||
		fclose(fp))
	{
		//perror(hof_path);
	}
...
void __pascal far hof_read() {
	...
	char custom_hof_path[POP_MAX_PATH];
	const char* hof_path = get_hof_path(custom_hof_path, sizeof(custom_hof_path));
	fp = fopen(hof_path, "rb");
	if (fp == nullptr)
		return;
	if (fread(&hof_count, 2, 1, fp) != 2 ||
		fread(&hof, sizeof(hof), 1, fp) != sizeof(hof)) {
		//perror(hof_path);
		hof_count = 0;
	}
	fclose(fp);
