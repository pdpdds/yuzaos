...
// * Does not assume eastl::allocator are templated with the Functor type.  Instead requests raw bytes from
// eastl::allocator and caches functor types itself in function_table.
...
#define FUNC_TEMPLATE_NOEXCEPT(FUNCTOR, ALLOCATOR) \
	EA_NOEXCEPT_IF(EA_NOEXCEPT_EXPR((detail::is_inplace_allocated<FUNCTOR, ALLOCATOR>::value)))
...
#define EASTL_FUNCTION_DEFAULT_NAME \
	EASTL_DEFAULT_NAME_PREFIX " function" // Unless the user overrides something, this is "EASTL function".
#endif
...
template <typename>
struct force_function_heap_allocation : public eastl::false_type
{
};
...
namespace detail
{
	...
	struct function_table_inplace_specialization
	{
		...
		static void destroy_allocator(total_storage_type& storage) EA_NOEXCEPT { get_allocator(storage).~Allocator(); }
	};
	...
	struct function_table_inplace_specialization<
	    T,
	    Allocator,
	    typename eastl::enable_if<!is_inplace_allocated<T, Allocator>::value>::type>
	{
		typedef T Functor;
		...
		EA_FORCE_INLINE static Result call(const functor_storage_type& storage, Arguments... arguments)
		{
			// do not call get_functor_ptr_ref because I want this function to be fast in debug when nothing gets
			// inlined
		...
		static void store_functor(total_storage_type& self, T to_store)
		{
			...
			FunctorPointer* ptr = new (&get_functor_ptr_ref(self)) FunctorPointer(reinterpret_cast<FunctorPointer>(
			    eastl::allocator_traits<Allocator>::allocate(allocator, sizeof(Functor))));
		...
		{
#if EASTL_INTERNAL_FUNCTION_ARE_TYPETRAITS_FUNCTIONAL
			static_assert(eastl::is_nothrow_move_constructible<FunctorPointer>::value,
		...
		static void destroy_functor(Allocator& allocator, total_storage_type& storage) EA_NOEXCEPT
		{
			...
			eastl::allocator_traits<Allocator>::deallocate(
			    allocator, reinterpret_cast<typename eastl::allocator_traits<Allocator>::pointer>(pointer),
			    sizeof(Functor));
		}
		...
		static void store_allocator(total_storage_type& storage, Allocator&& allocator)
		{
			...
			eastl::allocator_traits<Allocator>::construct(allocator, (Allocator*)storage.allocator_storage.first,
			                                              allocator);
		}
		...
		static void destroy_allocator(total_storage_type& storage) EA_NOEXCEPT
		{
			...
			eastl::allocator_traits<Allocator>::deallocate(
			    local_allocator, reinterpret_cast<typename eastl::allocator_traits<Allocator>::pointer>(&allocator),
			    sizeof(Allocator));
			eastl::allocator_traits<Allocator>::destroy(
			    local_allocator, reinterpret_cast<typename eastl::allocator_traits<Allocator>::pointer>(&allocator));
		}
	};
	...
	struct function_table
	{
		...
		{
			return function_table(&templated_call_move_and_destroy<T, Allocator>, &templated_call_copy<T, Allocator>,
			                      &templated_call_copy_functor_only<T, Allocator>, &templated_call_destroy<T, Allocator>
#ifndef FUNC_NO_RTTI
			                      ,
			                      &templated_call_type_id<T, Allocator>, , &templated_call_target<T, Allocator>
#endif
			);
		}
	...
	{
		static const function_table default_function_table =
		    function_table::create_default_function_table<T, Allocator>();
...
{
public:
	function() EA_NOEXCEPT { initialize_empty(); }
	function(std::nullptr_t) EA_NOEXCEPT { initialize_empty(); }
	...
	template <typename T>
	function(T functor,
	         typename eastl::enable_if<detail::is_valid_function_argument<T, Result(Arguments...)>::value,
	...
	function(eastl::allocator_arg_t,
	...
	template <typename Allocator>
	function(eastl::allocator_arg_t, const Allocator& allocator, const function& other) : call(other.call)
	{
		...
		detail::function_table_storage_type function_table_for_allocator =
		    &detail::get_default_function_table<typename eastl::allocator_traits<Allocator>::value_type, Allocator>();
		if (other.storage.function_table == function_table_for_allocator)
		{
			...
			detail::function_table_inplace_specialization<typename eastl::allocator_traits<Allocator>::value_type,
			                                              Allocator>::store_allocator(storage, Allocator(allocator));
		...
		{
			detail::function_table_storage_type function_table_for_function =
			    &detail::get_default_function_table<function, MyAllocator>();
			if (other.storage.function_table == function_table_for_function)
			{
				detail::init_function_table<function, MyAllocator>(storage);
				detail::function_table_inplace_specialization<function, Allocator>::store_allocator(
				    storage, MyAllocator(allocator));
	...
	~function() EA_NOEXCEPT { storage.function_table->call_destroy(storage); }
...
#ifndef FUNC_NO_RTTI
	const std::type_info& target_type() const EA_NOEXCEPT { return storage.function_table->call_type_id(); }
	template <typename T>
...
private:
	...
	{
		call =
		    &detail::function_table_inplace_specialization<typename T, Allocator>::template call<Result, Arguments...>;
		detail::init_function_table<T, Allocator>(storage);
		detail::function_table_inplace_specialization<T, Allocator>::store_allocator(
		    storage, EASTL_FORWARD(Allocator, allocator));
	...
	void initialize_empty() EA_NOEXCEPT
	{
		...
		// static_assert(detail::is_inplace_allocated<Empty_Function_Type, Allocator>::value, "The empty function should
		// benefit from small functor optimization");

		detail::init_function_table<Empty_Function_Type, Allocator>(storage);
		detail::function_table_inplace_specialization<Empty_Function_Type, Allocator>::store_allocator(storage,
		                                                                                               Allocator());
...
//
template <typename Result>
class function<Result()> : public detail::typedeffer<Result>
{
public:
	function() EA_NOEXCEPT { initialize_empty(); }
	...
	template <typename T>
	function(T functor,
	         typename eastl::enable_if<detail::is_valid_function_argument<T, Result()>::value,
	                                   detail::empty_struct>::type = detail::empty_struct())
	    FUNC_TEMPLATE_NOEXCEPT(T, EASTLAllocatorType)
	{
		if (detail::is_null(functor))
	...
	function(eastl::allocator_arg_t, const Allocator& allocator, const function& other) : call(other.call)
	{
		...
		else
		{
			...
			{
				detail::init_function_table<function, MyAllocator>(storage);
				detail::function_table_inplace_specialization<T, MyAllocator>::store_allocator(storage,
	...
	Result operator()() const { return call(storage.functor); }
...
//
template <typename Result, typename Argument0>
class function<Result(Argument0)> : public detail::typedeffer<Result, Argument0>
{
public:
	function() EA_NOEXCEPT { initialize_empty(); }
	...
	template <typename T>
	function(T functor,
	         typename eastl::enable_if<detail::is_valid_function_argument<T, Result(Argument0)>::value,
	                                   detail::empty_struct>::type = detail::empty_struct())
	    FUNC_TEMPLATE_NOEXCEPT(T, EASTLAllocatorType)
	{
		if (detail::is_null(functor))
	...
	function(eastl::allocator_arg_t, const Allocator& allocator, const function& other) : call(other.call)
	{
		...
		else
		{
			...
			{
				detail::init_function_table<function, MyAllocator>(storage);
				detail::function_table_inplace_specialization<T, Allocator>::store_allocator(storage,
				                                                                             MyAllocator(allocator));
				function_table_for_function->call_copy_functor_only(storage, other.storage);
			}
			else
	...
	Result operator()(Argument0 argument0) const { return call(storage.functor, EASTL_FORWARD(Argument0, argument0)); }
...
//
// This type trait is used by std::scoped_allocator_adaptor and may be used by custom allocators to determine whether
// the object being constructed is itself capable of using an allocator (e.g. is a container), in which case an
// allocator should be passed to its constructor. reference:  http://en.cppreference.com/w/cpp/memory/uses_allocator
// namespace eastl
// {
