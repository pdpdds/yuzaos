...
// 파일 시스템 임시 버퍼
BYTE gs_vbTempBuffer[ FILESYSTEM_SECTORSPERCLUSTER * 512 ];
DWORD g_mutex = 0;
 ...
 *  디렉터리를 엶
 */
MINT_DIR* kOpenDirectory( const char* pcDirectoryName )
{
	MINT_DIR* pstDirectory;
    DIRECTORYENTRY* pstDirectoryBuffer;
    
	Syscall_LockMutex(g_mutex);
    
    // 루트 디렉터리 밖에 없으므로 디렉터리 이름은 무시하고 핸들만 할당받아서 반환
    pstDirectory = (MINT_DIR*)kAllocateFileDirectoryHandle();
 ...
 */
struct kDirectoryEntryStruct* kReadDirectory(MINT_DIR* pstDirectory )
{
 ...
 */
void kRewindDirectory(MINT_DIR* pstDirectory )
{
    DIRECTORYHANDLE* pstDirectoryHandle;
 ...
 *  열린 디렉터리를 닫음
 */
int kCloseDirectory(MINT_DIR* pstDirectory )
{
    DIRECTORYHANDLE* pstDirectoryHandle;
