...
GUI_TopBar::GUI_TopBar(GUI_View *parent, const char *title,
                       eastl::function<void(GUI_View*)>callbackFunction ) :
    GUI_View( parent, title, 0, 0, -1, GUI_GetAppTopBarHeight()+GUI_GetStatusBarHeight(), nullptr),
titleView(NULL)
{
    ...
    contentView = GUI_View::create(this, "TopBarContent", 0, 0, -1, GUI_GetAppTopBarHeight(), nullptr);
    contentView->setAlign(GUI_ALIGN_LEFT|GUI_ALIGN_BOTTOM);
    contentView->setBorder( 0 );
    contentView->setBackgroundColor( cClear );
    
    if( title ) {
        titleView = GUI_Label::create(contentView, title, 0, 0, 0, 0, 0, nullptr);
