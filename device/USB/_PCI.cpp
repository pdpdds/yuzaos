...
static uint32_t config_read(uint8_t bus, uint8_t device, uint8_t func, uint8_t reg_off, uint8_t length)
{
	...
	Syscall_OutPortDWord(PCI_CONFIGURATION_ADDRESS,
             0x80000000
             | (bus    << 16)
             | (device << 11)
             | (func   <<  8)
    ...
    uint32_t readVal = Syscall_InPortDWord(PCI_CONFIGURATION_DATA) >> (8 * offset);
...
static void config_write_byte(uint8_t bus, uint8_t device, uint8_t func, uint8_t reg, uint8_t val)
{
    ...
    Syscall_OutPortByte(PCI_CONFIGURATION_DATA + (reg & 0x03), val);
	Syscall_UnlockMutex(pci_mutex);
}
...
static void config_write_word(uint8_t bus, uint8_t device, uint8_t func, uint8_t reg, uint16_t val)
{
    ...
    Syscall_OutPortWord(PCI_CONFIGURATION_DATA + (reg & 0x02), val);
	Syscall_UnlockMutex(pci_mutex);
}
...
static void config_write_dword(uint8_t bus, uint8_t device, uint8_t func, uint8_t reg, uint32_t val)
{
    ...
    Syscall_OutPortDWord(PCI_CONFIGURATION_DATA, val);
	Syscall_UnlockMutex(pci_mutex);
}
...
{
    printf("baseAddress address : %x, memory type : %x, memory size : %x\n", bar->baseAddress, bar->memoryType, bar->memorySize);
	...
	va = Syscall_MapPhysicalMemory("MemoryForMMIO", bar->baseAddress - offset, alignUp(bar->memorySize + offset, PAGE_SIZE), USER_READ | USER_WRITE | SYSTEM_READ | SYSTEM_WRITE, 0xd0000000 );
	va += offset;
	printf("va %x %x %x\n", va, bar->baseAddress - offset, alignUp(bar->memorySize + offset, PAGE_SIZE));
    Syscall_Sleep(1000);
