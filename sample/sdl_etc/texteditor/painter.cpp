...
Painter::~Painter()
{
  for (auto &i: glyphCache_)
    SDL_DestroyTexture(eastl::get<0>(i.second));
  TTF_CloseFont(font_);
}
...
void Painter::renderGlyph(wchar_t ch, int x, int y, Color fg, Color bg)
{
  SDL_Texture *texture;
  int width, height;
  auto iter = glyphCache_.find(eastl::make_tuple(ch, fg, bg));
  if (iter != end(glyphCache_))
  {
    texture = eastl::get<0>(iter->second);
    width = eastl::get<1>(iter->second);
    height = eastl::get<2>(iter->second);
    glyphCacheAge_.erase(eastl::get<3>(iter->second));
    eastl::get<3>(iter->second) = glyphCacheAge_.insert(end(glyphCacheAge_), iter->first);
  }
  else
  {
    if (glyphCache_.size() > 4096)
    {
      ...
      for (size_t j = 0; j < itemsCount / 2; ++j)
      {
        ...
        SDL_DestroyTexture(eastl::get<0>(iter->second));
    ...
    if (texture == nullptr)
    {
      ...
      assert("CreateTexture");
      //throw eastl::runtime_error("CreateTexture");
    }
    width = surf->w;
    height = surf->h;
    SDL_FreeSurface(surf);
    GlyphCacheKey key = eastl::make_tuple(ch, fg, bg);
    glyphCache_.insert(eastl::make_pair(key, eastl::make_tuple(texture, width, height, glyphCacheAge_.insert(end(glyphCacheAge_), key))));
  }
  SDL_Rect rect { x, y, width, height };
