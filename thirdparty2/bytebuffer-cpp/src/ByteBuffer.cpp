...
void ByteBuffer::printInfo() {
	...
	std::cout << "ByteBuffer " << name.c_str() << " Length: " << (int)length << ". Info Print" << std::endl;
}

void ByteBuffer::printAH() {
	uint32_t length = buf.size();
	std::cout << "ByteBuffer " << name.c_str() << " Length: " << (int)length << ". ASCII & Hex Print" << std::endl;
...
void ByteBuffer::printAscii() {
	uint32_t length = buf.size();
	std::cout << "ByteBuffer " << name.c_str() << " Length: " << (int)length << ". ASCII Print" << std::endl;
	for(uint32_t i = 0; i < length; i++) {
		printf("%c ", buf[i]);
	}
	printf("\n");
...
void ByteBuffer::printHex() {
	uint32_t length = buf.size();
	std::cout << "ByteBuffer " << name.c_str() << " Length: " << (int)length << ". Hex Print" << std::endl;
	for(uint32_t i = 0; i < length; i++) {
		printf("0x%02x ", buf[i]);
	}
	printf("\n");
...
void ByteBuffer::printPosition() {
	uint32_t length = buf.size();
	std::cout << "ByteBuffer " << name.c_str() << " Length: " << (int)length << " Read Pos: " << (int)rpos << ". Write Pos: " << (int)wpos << std::endl;
}
#endif
