...
bool PakInterface::AddPakFile(const std::string& theFileName)
{
	...
	_PFILE* aFP = FOpen(theFileName.c_str(), "rb");
	if (aFP == NULL)
		return false;
...
_PFILE* PakInterface::FOpen(const char* theFileName, const char* anAccess)
{
...
int PakInterface::FClose(_PFILE* theFile)
{
...
int PakInterface::FSeek(_PFILE* theFile, long theOffset, int theOrigin)
{
	if (theFile->mRecord != NULL)
	{
...
int PakInterface::FTell(_PFILE* theFile)
{
	if (theFile->mRecord != NULL)
		return theFile->mPos;
	else
...
size_t PakInterface::FRead(void* thePtr, int theElemSize, int theCount, _PFILE* theFile)
{
	if (theFile->mRecord != NULL)
	{
...
int PakInterface::FGetC(_PFILE* theFile)
{
	if (theFile->mRecord != NULL)
	{
		for (;;)
...
int PakInterface::UnGetC(int theChar, _PFILE* theFile)
{
	if (theFile->mRecord != NULL)
	{
...
char* PakInterface::FGetS(char* thePtr, int theSize, _PFILE* theFile)
{
	if (theFile->mRecord != NULL)
	{
		int anIdx = 0;
...
int PakInterface::FEof(_PFILE* theFile)
{
	if (theFile->mRecord != NULL)
