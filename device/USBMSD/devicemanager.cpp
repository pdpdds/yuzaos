...
FS_ERROR deviceManager_analyzeDisk(disk_t* disk)
{
    ...
    FS_ERROR err = SectorReads(0, buffer, 1, disk); // first sector of partition
  
    if (err != CE_GOOD)
    {
        printf("SectorReads does not work! Error: %u", err);
        ...
        (buffer[510] == 0x55 && buffer[511] == 0xAA)) // ... but like a Master Boot Record (MBR)
    {
        ...
        printf("Found MBR (DiskID: %xh)\n", ((uint16_t*)buffer)[440/2]);
        ...
        for (uint8_t i=0; i < 4; i++) // four entries in partition table
        {
            ...
            {
                printf("Partion Entry Num %d: Start: %d  Size: %d\n", i, entries[i].startLBA, entries[i].sizeLBA);

                disk->partition[i]         = (partition_t*)malloc(sizeof(partition_t));
...
{
    return disk->type->writeSectors(sector, buffer, count, disk);
}
...
{
    return disk->type->readSectors(sector, buffer, count, disk);
}