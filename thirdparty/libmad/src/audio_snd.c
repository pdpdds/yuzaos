 ...
 */
static
void __int32__(unsigned char *ptr, unsigned long num)
{
  ptr[0] = num >> 24;
  ptr[1] = num >> 16;
  ptr[2] = num >>  8;
...
int config(struct audio_config *config)
{
  ...
  __int32__(&header[0],  AUDIO_FILE_MAGIC);		/* magic */
  __int32__(&header[4],  sizeof(header));		/* hdr_size */
  __int32__(&header[8],  AUDIO_UNKNOWN_SIZE);	/* data_size */
  __int32__(&header[12], AUDIO_ENCODING_MULAW_8);	/* encoding */
  __int32__(&header[16], config->speed);		/* sample_rate */
  __int32__(&header[20], config->channels);		/* channels */
