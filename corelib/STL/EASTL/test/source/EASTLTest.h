...
#pragma warning(disable : 4350) // for whatever reason, the push,0 above does not turn this warning off with vs2012.
                                // VC++ 2012 STL headers generate this. warning C4350: behavior change:
                                // 'std::_Wrap_alloc<_Alloc>::_Wrap_alloc(const std::_Wrap_alloc<_Alloc> &) throw()'
                                // called instead of
                                // 'std::_Wrap_alloc<_Alloc>::_Wrap_alloc<std::_Wrap_alloc<_Alloc>>(_Other &) throw()'
#endif
...
#include <stdio.h>
#include <stdlib.h>
#include <vector.h> // For the STD_STL_TYPE defines below.
...
#pragma warning( \
    default : 4056) // Floating-point constant arithmetic generates a result that exceeds the maximum allowable value
#pragma warning(default : 4061) // The enumerate has no associated handler in a switch statement
#pragma warning( \
    default : 4062) // The enumerate has no associated handler in a switch statement, and there is no default label
...
#pragma warning(default : 4289) // Nonstandard extension used : 'var' : loop control variable declared in the for-loop
...
#pragma warning(default : 4339) // 'type' : use of undefined type detected in CLR meta-data - use of this type may lead
...
#pragma warning(default : 4623) // 'derived class' : default constructor could not be generated because a base class
                                // default constructor is inaccessible
#pragma warning(default : 4625) // 'derived class' : copy constructor could not be generated because a base class copy
                                // constructor is inaccessible
#pragma warning(default : 4626) // 'derived class' : assignment operator could not be generated because a base class
                                // assignment operator is inaccessible
#pragma warning(default : 4628) // Digraphs not supported with -Ze. Character sequence 'digraph' not interpreted as
                                // alternate token for 'char'
...
#pragma warning( \
    default : 4686) // 'user-defined type' : possible change in behavior, change in UDT return calling convention
                    //#pragma warning(default: 4710)      // 'function' : function not inlined
//#pragma warning(default: 4786)      // 'identifier' : identifier was truncated to 'number' characters in the debug
// information
...
#pragma warning( \
    default : 4928) // Illegal copy-initialization; more than one user-defined conversion has been implicitly applied
...
/// Example usage:
///    EASTLTest_Rand rng;
///    eastl_size_t   x = rng();                      // Generate value in range of [0, 0xffffffff] (i.e. generate any
///    uint32_t) eastl_ssize_t  y = rng.Rand(1000);             // Generate value in range of [0, 1000) eastl_ssize_t  z
///    = rng.RandRange(-50, +30);    // Generate value in range of [-50, +30)
///
...
public:
	...
	eastl_size_t
	operator()() // Returns a pseudorandom value in range of [0, 0xffffffffffffffff)] (i.e. generate any eastl_size_t)
	{
		return Rand();
	}
	...
	eastl_size_t RandLimit(eastl_size_t nLimit) // Returns a pseudorandom value in range of [0, nLimit)
	{
		...
		return (Rand() % nLimit); // This results in an imperfect distribution, especially for the case of nLimit being
		                          // high relative to eastl_size_t.
	}

	eastl_ssize_t RandRange(eastl_ssize_t nBegin,
	                        eastl_ssize_t nEnd) // Returns a pseudorandom value in range of [nBegin, nEnd)
	{
...
struct RandGenT
{
	RandGenT(eastl_size_t nSeed) : mRand(nSeed) {}

	Integer operator()() { return (Integer)mRand.Rand(); }

	Integer operator()(eastl_size_t n) { return (Integer)mRand.RandLimit(n); }
...
struct TestObject
{
	...
	int64_t mId;          // Unique id for each object, equal to its creation number. This value is not coped from other
	                      // TestObjects during any operations, including moves.
	uint32_t mMagicValue; // Used to verify that an instance is valid and that it is not corrupted. It should alwasy be
	                      // kMagicValue.
	...
	// This constructor exists for the purpose of testing variadiac template arguments, such as with the emplace
	// container functions.
...
{
	size_t operator()(const TestObject& a) const { return static_cast<size_t>(a.mX); }
};
}
...
struct use_mX
{
	int operator()(const T& t) const { return t.mX; }
};
...
{
	size_t operator()(const TestObject& t) const { return (size_t)t.mX; }
};
...
inline bool operator==(const Align16& a, const Align16& b) { return (a.mX == b.mX); }
...
inline bool operator==(const Align32& a, const Align32& b) { return (a.mX == b.mX); }
...
struct hash<Align64>
{
	size_t operator()(const Align64& a) const { return static_cast<size_t>(a.mX); }
};
}
...
struct GenerateIncrementalIntegers
{
	...
	GenerateIncrementalIntegers(int x = 0) : mX(x) {}

	void reset(int x = 0) { mX = x; }

	T operator()() { return T(mX++); }
};
...
struct SetIncrementalIntegers
{
	...
	SetIncrementalIntegers(int x = 0) : mX(x) {}

	void reset(int x = 0) { mX = x; }

	void operator()(T& t) { t = T(mX++); }
};
...
int CompareContainers(const T1& t1,
                      const T2& t2,
                      const char* ppName,
                      ExtractValue1 ev1 = test_use_self<T1>(),
                      ExtractValue2 ev2 = test_use_self<T2>())
{
	int nErrorCount = 0;
...
void PrintSequence(InputIterator first,
                   InputIterator last,
...
public:
	...
	demoted_iterator() : mIterator() {}

	explicit demoted_iterator(const Iterator& i) : mIterator(i) {}

	demoted_iterator(const this_type& x) : mIterator(x.mIterator) {}
...
inline bool operator==(const demoted_iterator<Iterator1, IteratorCategory1>& a,
                       const demoted_iterator<Iterator2, IteratorCategory2>& b)
{
...
template <typename Iterator1, typename IteratorCategory1, typename Iterator2, typename IteratorCategory2>
inline demoted_iterator<Iterator1, IteratorCategory1> operator-(const demoted_iterator<Iterator1, IteratorCategory1>& a,
                                                                const demoted_iterator<Iterator2, IteratorCategory2>& b)
{
...
inline demoted_iterator<Iterator1, IteratorCategory1> operator+(
    typename demoted_iterator<Iterator1, IteratorCategory1>::difference_type n,
    const demoted_iterator<Iterator1, IteratorCategory1>& a)
{
	return a + n;
}
...
inline demoted_iterator<Iterator, EASTL_ITC_NS::input_iterator_tag> to_input_iterator(const Iterator& i)
{
	return demoted_iterator<Iterator, EASTL_ITC_NS::input_iterator_tag>(i);
}

template <typename Iterator>
inline demoted_iterator<Iterator, EASTL_ITC_NS::forward_iterator_tag> to_forward_iterator(const Iterator& i)
{
	return demoted_iterator<Iterator, EASTL_ITC_NS::forward_iterator_tag>(i);
}

template <typename Iterator>
inline demoted_iterator<Iterator, EASTL_ITC_NS::bidirectional_iterator_tag> to_bidirectional_iterator(const Iterator& i)
{
	return demoted_iterator<Iterator, EASTL_ITC_NS::bidirectional_iterator_tag>(i);
}

template <typename Iterator>
inline demoted_iterator<Iterator, EASTL_ITC_NS::random_access_iterator_tag> to_random_access_iterator(const Iterator& i)
{
...
public:
	...
	void* allocate(size_t n, int = 0) { return new char[n]; }

	// We don't support alignment, so you can't use this class where alignment is required.
	void* allocate(size_t n, size_t, size_t, int = 0) { return new char[n]; }
...
//
//         list0 = list1; // list0 cannot free it's current contents with list1's allocator, and InstanceAllocator's
//         purpose is to detect if it mistakenly does so.
//
...
inline bool operator==(const InstanceAllocator& a, const InstanceAllocator& b)
{
