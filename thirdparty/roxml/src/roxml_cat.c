...
static void display_usage(const char* prgname)
{
	...
	printf("%s [options] <FILE> [<XPATH>]\n", prgname);
	printf("\tShow a specific XML subtree from a file\n");
	printf("options:    --help|-h                display help/usage\n");
	printf("options:    --quiet|-q               don't output errors\n");
	printf("\n");
	printf("Notes\n");
	printf("\tthe XPATH argument is OPTIONAL. If it's not specified, the function\n");
	printf("\tshows the complete XML tree ; otherwise, it shows only the selected\n");
	printf("\tpart of the tree.\n");
	printf("\n");
}
...
int main(int ac, char *av[])
{
	...
	while ((opt = getopt_long(ac, av, short_options, long_options, &longopt)) != -1) {
		...
		case 1:
			...
			if (!xmlname) {
				...
				printf("too many non-option - remove '%s'\n", av[optind - 1]);
				return EXIT_FAILURE;
			}
			break;
		case 'h':
	...
	if (!xmlname) {
		if ((flags & ROCAT_FLAG_QUIET) == 0) {
			printf("invalid options: no XML file name\n\n");
			display_usage(av[0]);
		}
		return EXIT_FAILURE;
	}
	...
	if (xpquery) {
		...
		if (!xpr) {
			if ((flags & ROCAT_FLAG_QUIET) == 0)
				printf("XPath query '%s' failed on '%s'\n", 
					xpquery, xmlname);
			goto on_error;
		}
		...
		if (xncount) {
			...
			if ((flags & ROCAT_FLAG_QUIET) == 0)
				printf("XPath query '%s' does not yield to any result\n",
					xpquery);
			goto on_error;
		}
