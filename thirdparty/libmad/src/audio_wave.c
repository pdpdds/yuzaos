 ...
 */
static
void __int32___(unsigned char *ptr, unsigned long num)
{
  ptr[0] = num >>  0;
  ptr[1] = num >>  8;
  ptr[2] = num >> 16;
...
int config(struct audio_config *config)
{
  ...
  __int32___(&chunk[4],  16);		/* chunkSize */

  int16(&chunk[8],  WAVE_FORMAT_PCM);	/* wFormatTag */
  int16(&chunk[10], config->channels);	/* wChannels */
  __int32___(&chunk[12], config->speed);	/* dwSamplesPerSec */
  __int32___(&chunk[16], bytes_ps);		/* dwAvgBytesPerSec */
