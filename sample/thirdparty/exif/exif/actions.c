...
convert_arg_to_entry (const char *set_value, ExifEntry *e, ExifByteOrder o, ExifLog *log)
{
	...
	for (i = 0; i < numcomponents; ++i) {
		...
		case EXIF_FORMAT_LONG:
			exif_set_long (e->data + (s * i), o, (long)atoi (value_p));
			break;
		...
		case EXIF_FORMAT_SLONG:
			exif_set_slong (e->data + (s * i), o, (long)atoi(value_p));
			break;
...
#define ENTRY_NOT_FOUND "   -   "
	...
	printf ("%s %d", txt, fieldwidth-(int)width);
	...
	for (i = (ExifIfd)0; i < EXIF_IFD_COUNT; i++) {
		...
		printf ("%s %d", exif_ifd_get_name (i), space - space/2);
	}
	fputc ('\n', stdout);
	...
	for (tag = 0; tag < 0xffff; tag++) {
		...
		printf ("0x%04x %s %d", tag, C(name), fieldwidth-(int)width);
...
show_entry_list (ExifEntry *e, void *data)
{
	...
	if (p->use_ids)
		...
		printf ("%s %d", str, fieldwidth-(int)width);
	}
	printf ("|");
...
action_tag_list (ExifData *ed, ExifParams p)
{
	...
	printf ("%s%d", s, fieldwidth-(int)width);
	fputc ('|', stdout);
