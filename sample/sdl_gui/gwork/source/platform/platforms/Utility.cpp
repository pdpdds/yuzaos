...
int vsnprintf_gwk(char* _str, size_t _count, const char* _format, va_list _argList)
{
	//20200102
/*#if defined(_MSC_VER)
...
#else*/
    vsnprintf(_str, _count, _format, _argList);
	return strlen(_str);
//#endif
}
...
int snprintf(char* _str, size_t _count, const char* _format, ...)
{
    ...
    const int len = vsnprintf_gwk(_str, _count, _format, argList);
    va_end(argList);
    return len;
}
...
void PrintfVargs(eastl::string& _out, const char* _format, va_list _argList)
{
    char temp[2048];

    char* out = temp;
    int len = Utility::vsnprintf_gwk(out, sizeof(temp), _format, _argList);
...
void Printf(eastl::string& _out, const char* _format, ...)
{
    va_list argList;
    va_start(argList, _format);
...
eastl::wstring Widen(const Gwk::String &nstr)
{
#ifndef AVOID_CPP11_CODECVT
...
Gwk::String Narrow(const eastl::wstring &wstr)
{
#ifndef AVOID_CPP11_CODECVT
    // wide to UTF-8 (C++11)
...
{
	eastl::transform(str.begin(), str.end(), str.begin(), toupper);
}
