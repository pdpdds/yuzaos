...
eastl::wstring SaveDialog::preInsert(Coord &cursor, const eastl::wstring &value)
{
  if (cursor.y == 1 && value.find(L"\n") == eastl::wstring::npos)
    return value;
  else if (value.find(L"\n") != eastl::wstring::npos && cursor.y > 0)
  {
    struct stat buf;
...
{
  buffer_.clear();
  char *tmp = getcwd(nullptr, MAXPATH);
  auto currentDir = toUtf16(tmp);
  free(tmp);
  setName(eastl::wstring{begin(currentDir) + currentDir.rfind(L'/'), end(currentDir)});
  ...
  closedir(d);
  eastl::sort(begin(buffer_) + 2, end(buffer_));
}
