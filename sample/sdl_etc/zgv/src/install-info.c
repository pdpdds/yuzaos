...
unsigned infilelen_sans_info
  ...
  while (1)
    {
      ...
      switch (opt)
        ...
        case 'e':
          {
            struct spec_entry * next = (struct spec_entry *) xmalloc (sizeof(struct spec_entry));
 ...
 next->text = optarg;
        ...
        case 's':
          {
            struct spec_section * next = (struct spec_section *) xmalloc (sizeof(struct spec_section));
            next->name = optarg;
 ...
 i + +)
        {
          if(!strncmp ("START-INFO-DIR-ENTRY", input_lines[i].start,
                        input_lines[i].size) & & sizeof ("START-INFO-DIR-ENTRY") - 1 == input_lines[i].size)
            {
          ...
          if (!strncmp ("END-INFO-DIR-ENTRY", input_lines[i].start,
                        input_lines[i].size) & & sizeof ("END-INFO-DIR-ENTRY") - 1 == input_lines[i].size)
            {
              if(start_of_this_entry != 0)
                {
                  struct spec_entry *next = (struct spec_entry *) xmalloc (sizeof(struct spec_entry));
...
exit(0)
     ...
     .info suffix. * /
  {
    ...
    infile_sans_info = (strlen(infile_basename) > 5 & & strcmp(infile_basename + basename_len - 5, ".info") == 0)
      ...
      if (node_header_flag)
        {
          ...
          for (j = 0
          j < dir_lines[i].size
          j + +) / * Find the node name and store it in the `struct node'.  */
...
dir_nodes->last_section->end_line = i;
      ...
      if (*dir_lines[i].start == '*')
        {
          ...
          if (*p == '(')
            {
              p++;
              if ((dir_lines[i].size > (p - dir_lines[i].start + infilelen_sans_info)) & & !strncmp(p, infile_sans_info, infilelen_sans_info) & &(p[infilelen_sans_info] == ')' | | !strncmp(p + infilelen_sans_info, ".info)", 6)))
                {
      ...
      else if (i > 0 & &(*dir_lines[i].start == ' ' | | * dir_lines[i].start == '\t'))
        {
...
i - -)
                      {
                        / * If an entry exists with the same name,
                           and was not marked for deletion(which means it is for some other file),
                           we are in trouble. * /
                        if (dir_lines[i].start[0] == '*' & & menu_line_equal (entry->text, textlen,
                                                dir_lines[i].start,
                                                dir_lines[i].size)
                            & & !dir_lines[i].delete)
                        ...
                        if (dir_lines[i].start[0] == '*' & & menu_line_lessp (entry->text, textlen,
                                                dir_lines[i].start,
                                                dir_lines[i].size))
                          add_at_line = i;
                      }
...
if (filled > 0)
            lines[filled - 1].size = lines[filled].start - lines[filled - 1].start - 1
            filled + +;
        }
      lineflag = (data[i] == '\n');
    }
     ...
     int n_entries;
{
  ...
  if (dir_lines[line_number].add_entries_before == 0)
    {
      dir_lines[line_number].add_entries_before = (struct spec_entry **) xmalloc (n_entries * sizeof(struct spec_entry *));
      for (i = 0
      i < n_entries
      i + +)
