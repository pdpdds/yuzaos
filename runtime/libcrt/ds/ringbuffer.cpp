...
RingBuffer_t *RingBufferCreate(size_t Size)
{
	...
	RingBuffer->Buffer = (uint8_t*)malloc(Size);
...
size_t RingBufferUnread(RingBuffer_t * ring_buffer) 
{
	if (ring_buffer->IndexRead == ring_buffer->IndexWrite) {
		return 0;
	}
	if (ring_buffer->IndexRead > ring_buffer->IndexWrite) {
		return (ring_buffer->Length - ring_buffer->IndexRead) + ring_buffer->IndexWrite;
	}
	else {
		return (ring_buffer->IndexWrite - ring_buffer->IndexRead);
	}
}
