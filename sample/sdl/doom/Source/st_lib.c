  ...
  boolean refresh )
{
  ...
  int   w = SHORT_(n->p[0]->width);
  int   h = SHORT_(n->p[0]->height);
  int   x = n->x;
  ...
  boolean   refresh )
{
  ...
  if (*mi->on && (mi->oldinum != *mi->inum || refresh))
  {
    if (mi->oldinum != -1)
    {
      x = mi->x - SHORT_(mi->p[mi->oldinum]->leftoffset);
      y = mi->y - SHORT_(mi->p[mi->oldinum]->topoffset);
      w = SHORT_(mi->p[mi->oldinum]->width);
      h = SHORT_(mi->p[mi->oldinum]->height);
  ...
  boolean   refresh )
{
  ...
  {
    x = bi->x - SHORT_(bi->p->leftoffset);
    y = bi->y - SHORT_(bi->p->topoffset);
    w = SHORT_(bi->p->width);
    h = SHORT_(bi->p->height);
